use comunicado::ui::status_bar::{
    StatusBar, StatusSegment, EmailStatusSegment, CalendarStatusSegment, 
    SystemInfoSegment, NavigationHintsSegment, SyncStatus, StatusBarPosition, SeparatorStyle
};
...
fn test_navigation_hints_segment() {
    ...
    assert_eq!(segment.content(), "Folders | Tab: Switch | j/k: Navigate | l: Expand");
...
fn test_navigation_hints_segment_many_shortcuts() {
    ...
    assert_eq!(segment.content(), "Messages | Tab: Switch | j/k: Navigate | Enter: Open");
}

#[test]
fn test_status_bar_creation() {
    let status_bar = StatusBar::new(StatusBarPosition::Bottom);
    assert_eq!(status_bar.get_status_summary(), "StatusBar: 0 segments, position: Bottom, style: Powerline");
}
...
fn test_status_bar_add_segments() {
    ...
    assert_eq!(status_bar.get_status_summary(), "StatusBar: 2 segments, position: Top, style: Powerline");
}
...
fn test_status_bar_remove_segment() {
    ...
    status_bar.add_segment("email".to_string(), email_segment);
    assert_eq!(status_bar.get_status_summary(), "StatusBar: 1 segments, position: Bottom, style: Powerline");
...
fn test_status_bar_segment_ordering() {
    ...
    let order = vec!["email".to_string(), "system".to_string(), "navigation".to_string()];
    status_bar.set_segment_order(order);
    
    assert_eq!(status_bar.get_status_summary(), "StatusBar: 3 segments, position: Bottom, style: Powerline");
}
...
fn test_status_bar_separator_styles() {
    ...
    assert_eq!(status_bar.get_status_summary(), "StatusBar: 0 segments, position: Top, style: Simple");
    
    status_bar.set_separator_style(SeparatorStyle::Minimal);
    assert_eq!(status_bar.get_status_summary(), "StatusBar: 0 segments, position: Top, style: Minimal");
    
    status_bar.set_separator_style(SeparatorStyle::Powerline);
    assert_eq!(status_bar.get_status_summary(), "StatusBar: 0 segments, position: Top, style: Powerline");
}
