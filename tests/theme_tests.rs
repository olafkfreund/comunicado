use comunicado::theme::{
    AccessibilityOptions, ColorBlindness, Theme, ThemeConfig, ThemeManager, UserPreferences,
};
...
fn test_color_blindness_support() {
    ...
    let _protanopia_theme = theme
        .colors
        .adjust_for_color_blindness(ColorBlindness::Protanopia);
    ...
    let _deuteranopia_theme = theme
        .colors
        .adjust_for_color_blindness(ColorBlindness::Deuteranopia);
    assert!(true);

    // Test tritanopia adjustment
    let _tritanopia_theme = theme
        .colors
        .adjust_for_color_blindness(ColorBlindness::Tritanopia);
    assert!(true);
}
