...
mod keyboard_shortcut_tests {
        ...
        for (key_code, modifiers, expected_desc) in test_cases {
            match manager.get_action(key_code, modifiers) {
                ...
                None => {
                    println!(
                        "  ❌ {:?}+{:?} → NOT FOUND (expected: {})",
                        key_code, modifiers, expected_desc
                    );
                }
            }
        }
    ...
    fn test_v_shortcut_specifically() {
        ...
        let v_shortcuts: Vec<_> = all_shortcuts
            .iter()
        ...
        println!(
            "  Direct V shortcut test: {:?}",
            manager.config().get_action(&v_shortcut)
        );
    }
    ...
    fn test_keyboard_config_loading() {
        ...
        let has_quit = manager
            .get_action(KeyCode::Char('q'), KeyModifiers::NONE)
            .is_some();
        let has_help = manager
            .get_action(KeyCode::Char('?'), KeyModifiers::NONE)
            .is_some();
        ...
        if !has_quit || !has_help {
            println!(
                "  ❌ Basic shortcuts missing - setup_default_shortcuts() may not have been called"
            );
        } else {
