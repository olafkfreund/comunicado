...
use comunicado::email::{AttachmentType, AttachmentInfo, StoredAttachment};
...
fn test_gmail_attachment_scenarios() {
    ...
    for (filename, content_type, expected_type, expected_display_name) in test_cases {
        ...
        assert_eq!(info.attachment_type, expected_type, 
                  "Gmail attachment type detection failed for {}", filename);
        assert_eq!(info.display_name, expected_display_name,
                  "Gmail filename decoding failed for {}", filename);
...
fn test_outlook_attachment_scenarios() {
    ...
    for (filename, content_type, expected_type) in test_cases {
        ...
        assert_eq!(info.attachment_type, expected_type,
                  "Outlook attachment type detection failed for {}", filename);
        assert_eq!(info.display_name, filename,
...
fn test_yahoo_attachment_scenarios() {
    ...
    for (filename, content_type, expected_type) in test_cases {
        ...
        assert_eq!(info.attachment_type, expected_type,
                  "Yahoo attachment type detection failed for {}", filename);
    }
}
...
fn test_protonmail_attachment_scenarios() {
    ...
    for (filename, content_type, expected_type, expected_safe) in test_cases {
        ...
        assert_eq!(info.attachment_type, expected_type,
                  "ProtonMail attachment type detection failed for {}", filename);
        assert_eq!(info.is_safe, expected_safe,
                  "ProtonMail safety classification failed for {}", filename);
    }
}
...
fn test_international_filename_handling() {
    ...
    let test_cases = vec![
        ...
        // German with umlauts  
        ("=?UTF-8?Q?M=C3=BCnchen_Pr=C3=A4sentation.pptx?=", "München_Präsentation.pptx"),
        // French with accents
        ("=?ISO-8859-1?Q?r=E9sum=E9.pdf?=", "résumé.pdf"),
        // Chinese
        ("=?UTF-8?B?5paH5qGjLnR4dA==?=", "文档.txt"),
        // Russian
        ("=?UTF-8?B?0LTQvtC60YPQvNC10L3RgiDRgNGD0YHRgdC60LjQuS5wZGY=?=", "документ русский.pdf"),
        // Mixed ASCII and international
        ("=?UTF-8?Q?Report_2024_=E2=80=93_M=C3=BCnchen.xlsx?=", "Report_2024_–_München.xlsx"),
    ];
    ...
    for (encoded, expected_decoded) in test_cases {
        ...
        assert_eq!(info.display_name, expected_decoded,
                  "International filename decoding failed for: {}", encoded);
    }
}
...
fn test_content_type_edge_cases() {
    ...
    let test_cases = vec![
        ...
        ("document.pdf", "application/octet-stream", AttachmentType::Pdf),
        ("photo.jpg", "application/binary", AttachmentType::Jpeg),
        
        // Both unknown - should be unknown
        ("unknown_file", "application/octet-stream", AttachmentType::Unknown),
...
fn test_mime_header_edge_cases() {
    ...
    for (encoded, expected) in test_cases {
        ...
        assert_eq!(decoded, expected, 
                  "MIME decoding failed for: {}", encoded);
    }
}
...
fn test_security_scenarios_by_provider() {
    ...
    for (filename, content_type) in dangerous_files {
        ...
        assert!(matches!(info.attachment_type, 
                        AttachmentType::Unknown | 
                        AttachmentType::Text |
                        AttachmentType::Zip |
                        AttachmentType::Tar),
               "Dangerous file {} should still have valid type detection", filename);
    }
}