...
use comunicado::oauth2::{AccountConfig, AuthType, SecurityType, TokenManager};
use std::sync::Arc;
...
async fn test_background_processing_channels() -> Result<()> {
    ...
    let _imap_manager = Arc::new(
        comunicado::imap::ImapAccountManager::new().expect("Failed to create IMAP manager"),
    );

    // Create background processor message channels
    let (_progress_sender, _progress_receiver) =
        tokio::sync::mpsc::unbounded_channel::<comunicado::email::sync_engine::SyncProgress>();
...
async fn test_sync_progress_reporting() {
    ...
    assert!(matches!(
        progress.phase,
        comunicado::email::sync_engine::SyncPhase::Initializing
    ));
}
