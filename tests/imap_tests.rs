use comunicado::imap::{
    ImapConfig, ImapClient, ImapCapability, ImapFolder, ImapMessage, 
    MessageFlag, SearchCriteria, FolderAttribute, ImapError
};
...
fn test_imap_capability_parsing() {
    assert_eq!(ImapCapability::from_str("IMAP4REV1"), ImapCapability::Imap4Rev1);
    assert_eq!(ImapCapability::from_str("STARTTLS"), ImapCapability::StartTls);
    assert_eq!(ImapCapability::from_str("AUTH=PLAIN"), ImapCapability::AuthPlain);
...
fn test_folder_attribute_parsing() {
    assert_eq!(FolderAttribute::from_str("\\HasChildren"), FolderAttribute::HasChildren);
    assert_eq!(FolderAttribute::from_str("\\Noselect"), FolderAttribute::Noselect);
    assert_eq!(FolderAttribute::from_str("\\Sent"), FolderAttribute::Sent);
    assert_eq!(FolderAttribute::from_str("\\Drafts"), FolderAttribute::Drafts);
...
fn test_message_flag_to_string() {
    ...
    assert_eq!(MessageFlag::Custom("MyFlag".to_string()).to_string(), "MyFlag");
}
...
"#;
    ...
    assert!(gmail_parent.attributes.contains(&FolderAttribute::HasChildren));
...
fn test_protocol_command_formatting() {
    ...
    assert_eq!(
        ImapProtocol::format_examine("Drafts"),
        "EXAMINE \"Drafts\""
    );
...
fn test_body_structure_checks() {
    ...
    let multipart = comunicado::imap::BodyStructure::new("multipart".to_string(), "mixed".to_string());
