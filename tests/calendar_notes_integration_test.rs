...
use comunicado::calendar::event::{AttendeeStatus, EventAttendee};
use comunicado::calendar::{Calendar, CalendarSource, Event};
use comunicado::plugins::notes::{
    CalendarNotesConfig, EventLinkType, Note, NoteStorage, WatchedDirectory,
};
...
async fn test_meeting_note_creation_logic() {
    ...
    let stored_dir = note_storage
        .add_watched_directory(calendar_dir)
        .await
        .unwrap();
...
async fn test_event_link_types_and_metadata() {
    ...
    let event_notes = note_storage
        .get_notes_by_tag(&format!("event:{}", event.id))
        .await
        .unwrap();
    assert_eq!(event_notes.len(), 5);

    // Test searching for specific link types
    let meeting_notes = note_storage
        .get_notes_by_tag("link-type:meetingnotes")
        .await
        .unwrap();
    assert_eq!(meeting_notes.len(), 1);

    let prep_notes = note_storage
        .get_notes_by_tag("link-type:preparation")
        .await
        .unwrap();
...
async fn test_integration_readiness() {
    ...
    note_storage
        .store_note(&calendar_note, stored_dir.id)
        .await
        .unwrap();
    ...
    let calendar_search = note_storage
        .search_notes("Generated from calendar event", 10)
        .await
        .unwrap();
    assert_eq!(calendar_search.len(), 1);

    let meeting_search = note_storage
        .search_notes("Meeting: Test Event", 10)
        .await
        .unwrap();
