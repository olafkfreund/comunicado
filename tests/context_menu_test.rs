...
use comunicado::ui::{ContextAwareMenu, MenuContext, UIMode, FocusedPane};
...
fn test_context_menu_esc_key_navigation() {
    ...
    assert!(!can_go_back, "navigate_back() should return false when at main menu");
    
    // Menu should still be visible after navigate_back returns false
    assert!(menu.is_visible(), "Menu should still be visible after failed navigate_back");
...
fn test_context_menu_esc_key_with_submenu() {
    ...
    assert!(!can_go_back_main, "navigate_back() should return false at main menu");
...
fn test_key_event_creation() {
    ...
    assert!(matches!(esc_key.code, KeyCode::Esc), "Key event should have Esc code");
    assert_eq!(esc_key.modifiers, KeyModifiers::NONE, "Key event should have no modifiers");
...
fn test_context_menu_full_cycle() {
    ...
    menu.navigate_up();
    menu.navigate_down();
    assert!(menu.is_visible(), "Menu should remain visible during navigation");
...
async fn test_context_menu_integration_with_key_simulation() {
    ...
    assert!(!menu.is_visible(), "Menu should be hidden after Esc key simulation");
