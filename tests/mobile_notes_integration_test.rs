...
use comunicado::plugins::notes::{Note, NoteStorage, WatchedDirectory};
...
async fn test_notes_and_mobile_systems_compatibility() {
    ...
    let dir = WatchedDirectory::new(temp_dir.path().join("notes"), "Test Notes".to_string());
...
async fn test_note_metadata_for_mobile_integration() {
    ...
    let contact_notes = note_storage
        .get_notes_by_tag("contact:john-doe")
        .await
        .unwrap();
    assert_eq!(contact_notes.len(), 1);

    // Test full-text search for mobile content
    let search_results = note_storage
        .search_notes("converted from SMS", 10)
        .await
        .unwrap();
...
async fn test_integration_readiness() {
    ...
    let note_storage = NoteStorage::new(temp_dir.path()).await.unwrap();
    let message_store = MessageStore::new(temp_dir.path().join("messages.db"))
        .await
        .unwrap();
    ...
    let stored_dir = note_storage
        .add_watched_directory(mobile_dir)
        .await
        .unwrap();
    ...
    note_storage
        .store_note(&mobile_note, stored_dir.id)
        .await
        .unwrap();
