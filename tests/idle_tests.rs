use comunicado::imap::{
    ImapClient, ImapConfig, ImapAccountManager, ImapAuthMethod, ImapCapability
};
...
fn test_idle_notification_types() {
    ...
    let fetch = IdleNotification::Fetch { sequence: 1, uid: Some(1234) };
...
fn test_idle_parsing_performance() { 
    ...
    println!("Parsed {} notifications in {:?}", notifications.len(), duration);
...
async fn example_idle_usage() {
    ...
    println!("IDLE capabilities: {:?}", client.capabilities().contains(&ImapCapability::Idle));
