use anyhow::Result;
use comunicado::email::sync_engine::{SyncEngine, SyncPhase, SyncProgress, SyncStrategy};
...
async fn test_background_processor_channels() -> Result<()> {
    ...
    let (_progress_sender, _progress_receiver) =
        tokio::sync::mpsc::unbounded_channel::<SyncProgress>();
    let (_completion_sender, _completion_receiver) = tokio::sync::mpsc::unbounded_channel::<
        comunicado::performance::background_processor::TaskResult,
    >();
...
async fn test_sync_progress_phases() {
    ...
    for phase in phases {
        ...
        match phase {
            ...
            SyncPhase::CheckingFolders => {
                assert!(matches!(progress.phase, SyncPhase::CheckingFolders))
            }
            SyncPhase::FetchingHeaders => {
                assert!(matches!(progress.phase, SyncPhase::FetchingHeaders))
            }
            SyncPhase::FetchingBodies => {
                assert!(matches!(progress.phase, SyncPhase::FetchingBodies))
            }
            SyncPhase::ProcessingChanges => {
                assert!(matches!(progress.phase, SyncPhase::ProcessingChanges))
            }
...
async fn test_sync_service_components() -> Result<()> {
    ...
    let _imap_manager = Arc::new(ImapAccountManager::new().expect("Failed to create IMAP manager"));
