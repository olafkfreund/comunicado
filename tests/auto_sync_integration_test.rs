...
mod tests {
    use comunicado::email::{
        ...
        sync_config::{AccountSyncSettings, SyncConfigFile, SyncConfigManager},
    };
    ...
    fn test_auto_sync_config_serialization() {
        ...
        let deserialized: AutoSyncConfig =
            serde_json::from_str(&json).expect("Failed to deserialize config");

        assert_eq!(deserialized.enabled, config.enabled);
        assert_eq!(
            deserialized.sync_interval_minutes,
            config.sync_interval_minutes
        );
        assert_eq!(deserialized.sync_on_startup, config.sync_on_startup);
        assert_eq!(
            deserialized.max_concurrent_syncs,
    ...
    fn test_sync_config_file_creation() {
        ...
        let deserialized: SyncConfigFile =
    ...
    fn test_sync_config_manager_basic() {
        ...
        let config_manager =
            SyncConfigManager::new(config_path).expect("Failed to create config manager");
    ...
    fn test_notification_persistence_config() {
        ...
        use comunicado::notifications::persistence::{NotificationStorage, PersistenceSettings};
