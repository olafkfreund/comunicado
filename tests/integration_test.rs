...
fn test_basic_attachment_type_detection() {
    ...
    assert_eq!(AttachmentType::from_content_type("image/png"), AttachmentType::Png);
    assert_eq!(AttachmentType::from_content_type("text/plain"), AttachmentType::Text);
...
fn test_filename_type_detection() {
    ...
    assert_eq!(AttachmentType::from_filename("image.PNG"), AttachmentType::Png); // Case insensitive
    assert_eq!(AttachmentType::from_filename("notes.txt"), AttachmentType::Text);
...
fn test_unsafe_attachment_detection() {
    ...
    for filename in dangerous_files {
        ...
        let info = AttachmentInfo::from_stored(stored);
        assert!(!info.is_safe, "File {} should be marked as unsafe", filename);
    }
}
...
fn test_attachment_descriptions_and_icons() {
    ...
    for attachment_type in types {
        ...
        assert!(!attachment_type.description().is_empty(), 
                "{:?} should have a description", attachment_type);
        
        // Each type should have a non-empty icon
        assert!(!attachment_type.icon().is_empty(), 
                "{:?} should have an icon", attachment_type);
    }
}
...
fn test_gmail_specific_content_types() {
    ...
    assert_eq!(
        AttachmentType::from_content_type("application/vnd.openxmlformats-officedocument.wordprocessingml.document"), 
        AttachmentType::Word
    );
    assert_eq!(
        AttachmentType::from_content_type("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"), 
        AttachmentType::Excel
    );
    assert_eq!(
        AttachmentType::from_content_type("application/vnd.openxmlformats-officedocument.presentationml.presentation"), 
...
fn test_size_formatting() {
    ...
    for (size_bytes, expected_format) in test_cases {
        ...
        if expected_format.contains("B") && !expected_format.contains("KB") {
            assert!(formatted.contains("B"), "Size {} should format to bytes", size_bytes);
        } else if expected_format.contains("KB") {
            assert!(formatted.contains("KB"), "Size {} should format to KB", size_bytes);
        } else if expected_format.contains("MB") {
            assert!(formatted.contains("MB"), "Size {} should format to MB", size_bytes);
        } else if expected_format.contains("GB") {
            assert!(formatted.contains("GB"), "Size {} should format to GB", size_bytes);
        }
    }
}
