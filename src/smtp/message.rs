...
impl MessageBuilder {
    ...
    pub fn build(self) -> SmtpResult<Message> {
        ...
        let from = self.from.ok_or_else(|| SmtpError::MessageFormatError("From address is required".to_string()))?;
        
        if self.to.is_empty() && self.cc.is_empty() && self.bcc.is_empty() {
            return Err(SmtpError::MessageFormatError("At least one recipient is required".to_string()));
        }
        
        if self.subject.is_empty() {
            return Err(SmtpError::MessageFormatError("Subject is required".to_string()));
        }
...
impl EmailMessage {
    ...
    pub fn create_reply(
        ...
        let mut reply = Self {
            ...
            cc: if reply_all { original.cc.clone() } else { Vec::new() },
            bcc: Vec::new(),
    ...
    pub fn validate(&self) -> SmtpResult<()> {
        if self.from.is_empty() {
            return Err(SmtpError::MessageFormatError("From address is required".to_string()));
        }
...
mod tests {
    ...
    fn test_parse_address_list() {
        let addresses = parse_address_list("test1@example.com, John <test2@example.com>, test3@example.com").unwrap();
    ...
    fn test_message_builder() {
        let message = MessageBuilder::new()
            .from_str("sender@example.com").unwrap()
            .to_str("recipient@example.com").unwrap()
