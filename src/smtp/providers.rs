use crate::smtp::{SmtpConfig, SmtpAuth, SmtpSecurity, SmtpResult, SmtpError};
...
impl SmtpProviderRegistry {
    ...
    pub fn new() -> Self {
        ...
        // Gmail
        providers.insert("gmail".to_string(), SmtpProviderConfig {
        ...
        // Outlook
        providers.insert("outlook".to_string(), SmtpProviderConfig {
            name: "outlook".to_string(),
            display_name: "Outlook/Hotmail".to_string(),
            smtp_server: "smtp-mail.outlook.com".to_string(),
            smtp_port: 587,
            security: SmtpSecurity::StartTls,
            auth_url: Some("https://login.microsoftonline.com/common/oauth2/v2.0/authorize".to_string()),
            token_url: Some("https://login.microsoftonline.com/common/oauth2/v2.0/token".to_string()),
            scopes: vec![
        ...
        // Yahoo
        providers.insert("yahoo".to_string(), SmtpProviderConfig {
        ...
        // iCloud
        providers.insert("icloud".to_string(), SmtpProviderConfig {
        ...
        // ProtonMail Bridge
        providers.insert("protonmail".to_string(), SmtpProviderConfig {
    ...
    pub fn create_smtp_config(
        ...
        let provider = self.get_provider(provider_name)
            .ok_or_else(|| SmtpError::InvalidConfig(format!("Unknown provider: {}", provider_name)))?;
    ...
    pub fn detect_provider_from_email(&self, email: &str) -> Option<&SmtpProviderConfig> {
        if let Some(domain) = email.split('@').nth(1) {
            match domain.to_lowercase().as_str() {
                ...
                "outlook.com" | "hotmail.com" | "live.com" | "msn.com" => self.get_provider("outlook"),
                "yahoo.com" | "yahoo.co.uk" | "yahoo.fr" | "ymail.com" => self.get_provider("yahoo"),
        ...
        match &config.auth {
            SmtpAuth::OAuth2 { username, access_token } => {
                if username.is_empty() {
                    return Err(SmtpError::InvalidConfig("Username cannot be empty".to_string()));
                }
                if access_token.is_empty() {
                    return Err(SmtpError::InvalidConfig("Access token cannot be empty".to_string()));
                }
            }
            SmtpAuth::Plain { username, password } | SmtpAuth::Login { username, password } => {
                ...
                if password.is_empty() {
                    return Err(SmtpError::InvalidConfig("Password cannot be empty".to_string()));
                }
            }
        }
...
mod tests {
    ...
    fn test_provider_registry() {
        ...
        assert_eq!(registry.detect_provider_from_email("user@gmail.com").unwrap().name, "gmail");
        assert_eq!(registry.detect_provider_from_email("user@outlook.com").unwrap().name, "outlook");
        assert_eq!(registry.detect_provider_from_email("user@yahoo.com").unwrap().name, "yahoo");
        assert!(registry.detect_provider_from_email("user@unknown.com").is_none());
    }
    ...
    fn test_smtp_config_creation() {
        ...
        let config = registry.create_smtp_config(
            "gmail",
    ...
    fn test_security_recommendations() {
        ...
        assert_eq!(utils::get_recommended_security(2525), SmtpSecurity::StartTls);
    }
}