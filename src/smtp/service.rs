use crate::email::{database::StoredDraft, EmailDatabase};
use crate::oauth2::TokenManager;
use crate::smtp::{
    EmailMessage, SendResult, SmtpClient, SmtpError, SmtpProviderRegistry, SmtpResult,
};
...
impl SmtpService {
    ...
    pub async fn initialize_account(
        ...
        let client =
            SmtpClient::new_with_oauth2(config, self.token_manager.as_ref().clone()).await?;
    ...
    pub async fn send_email(
        ...
        let email_message =
            EmailMessage::from_compose_data(compose_data, from_address.to_string())?;
        ...
        let client = clients.get(account_id).ok_or_else(|| {
            SmtpError::InvalidConfig(format!("No SMTP client found for account: {}", account_id))
        })?;
    ...
    pub async fn update_oauth2_token(&self, account_id: &str, new_token: &str) -> SmtpResult<()> {
        ...
        if let Some(client) = clients.get_mut(account_id) {
            ...
            Err(SmtpError::InvalidConfig(format!(
                "No SMTP client found for account: {}",
                account_id
            )))
        }
    }
    ...
    pub async fn save_draft(
        ...
        if compose_data.subject.is_empty() && compose_data.body.is_empty() {
            return Err(SmtpError::MessageFormatError(
                "Draft must have either subject or body".to_string(),
            ));
        }
        ...
        match self.database.load_draft(draft_id).await {
            Ok(Some(draft)) => {
                ...
                if draft.account_id != account_id {
                    return Err(SmtpError::InvalidConfig(format!(
                        "Draft {} does not belong to account {}",
                        draft_id, account_id
                    )));
                }
                Ok(draft.to_compose_data())
            }
            Ok(None) => Err(SmtpError::InvalidConfig(format!(
                "Draft not found: {}",
                draft_id
            ))),
            Err(e) => Err(SmtpError::InvalidConfig(format!(
    ...
    pub async fn delete_draft(&self, account_id: &str, draft_id: &str) -> SmtpResult<()> {
        ...
        match self.database.delete_draft(draft_id).await {
            ...
            Ok(false) => Err(SmtpError::InvalidConfig(format!(
                "Draft {} was not found during deletion",
                draft_id
            ))),
            Err(e) => Err(SmtpError::InvalidConfig(format!(
    ...
    pub async fn list_drafts(&self, account_id: &str) -> SmtpResult<Vec<StoredDraft>> {
        self.database
            .load_drafts_for_account(account_id)
            .await
    ...
    pub async fn update_draft(
        ...
        tracing::info!(
            "Draft updated for account {}: {} (auto_saved: {})",
            account_id,
            draft_id,
            auto_saved
        );
    ...
    pub async fn auto_save_draft(
        ...
        if let Some(draft_id) = existing_draft_id {
            ...
            self.update_draft(account_id, draft_id, compose_data, true)
                .await?;
            Ok(draft_id.to_string())
        } else {
            // Create new auto-saved draft
            let mut draft =
                StoredDraft::from_compose_data(account_id.to_string(), compose_data, true);
            draft.auto_saved = true;
            let draft_id = draft.id.clone();

            self.database.save_draft(&draft).await.map_err(|e| {
                SmtpError::InvalidConfig(format!("Failed to auto-save draft: {}", e))
            })?;
    ...
    pub async fn cleanup_old_auto_saved_drafts(&self, older_than_hours: i64) -> SmtpResult<u64> {
        self.database
            .cleanup_old_auto_saved_drafts(older_than_hours)
            .await
...
mod tests {
    ...
    async fn test_provider_detection() {
        ...
        assert_eq!(
            service.detect_provider_from_email("user@gmail.com"),
            Some("gmail".to_string())
        );
        assert_eq!(
            service.detect_provider_from_email("user@outlook.com"),
    ...
    async fn test_draft_operations() {
        ...
        let draft_id = service
            .save_draft("test_account", &compose_data)
            .await
            .unwrap();
        ...
        assert!(service
            .delete_draft("test_account", &draft_id)
            .await
            .is_ok());
    }
}
