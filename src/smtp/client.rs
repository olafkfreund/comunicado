...
use crate::smtp::{SendResult, SmtpAuth, SmtpError, SmtpResult, SmtpSecurity};
use lettre::{
    transport::smtp::{
...
impl SmtpClient {
    ...
    async fn build_transport(
        config: &SmtpConfig,
    ) -> SmtpResult<AsyncSmtpTransport<lettre::Tokio1Executor>> {
        let _server_addr = format!("{}:{}", config.server, config.port);

        // Create builder
        let mut builder =
            AsyncSmtpTransport::<lettre::Tokio1Executor>::builder_dangerous(&config.server)
                .port(config.port)
        ...
        match config.security {
            ...
            SmtpSecurity::StartTls => {
                let tls_parameters = TlsParameters::builder(config.server.clone())
                    ...
                    .build()
                    .map_err(|e| {
                        SmtpError::ConnectionFailed(format!("TLS configuration failed: {}", e))
                    })?;
        ...
        match result {
            Ok(_response) => {
                ...
                tracing::info!(
                    ...
                    send_result.accepted_recipients.len()
                );
        ...
        match self.send(message.clone()).await {
            ...
            Err(SmtpError::AuthenticationFailed(_)) | Err(SmtpError::NetworkError(_)) => {
                ...
                if let Some(ref token_manager) = self.token_manager {
                    tracing::info!(
                        "SMTP authentication failed, attempting token refresh for account: {}",
                        account_id
                    );
    ...
    pub async fn update_oauth2_token(&mut self, new_token: String) -> SmtpResult<()> {
        ...
        } = &mut self.config.auth
        {
            *access_token = new_token;
            ...
            Err(SmtpError::InvalidConfig(
                "Not configured for OAuth2 authentication".to_string(),
            ))
        }
    }
}
...
mod tests {
    ...
    fn test_smtp_config_creation() {
        let config =
            SmtpConfig::gmail_oauth2("test@gmail.com".to_string(), "fake_token".to_string());
