...
use crate::ui::components::{AppMode, EventResult, ModularUI, UIEvent};
...
pub async fn test_modular_ui_basic() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "📊 Basic metrics: {} components, {} events",
...
pub async fn test_modular_ui_with_minimal_init() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "📊 After stress test: {} events processed",
...
pub async fn test_modular_ui_performance() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut ui = ModularUI::new()?;
    ui.initialize(None, None, None, None, None, None, None)
        .await?;
    ...
    println!(
        "   Event processing time: {:.2}ms",
        event_processing_time.as_secs_f64() * 1000.0
    );
    println!(
        "   Average per event: {:.2}μs",
        event_processing_time.as_micros() as f64 / 100.0
    );
    println!(
    ...
    if avg_event_time < 1000.0 {
        // Less than 1ms per event
        println!("✅ Performance is excellent (<1ms per event)");
    } else {
        println!(
            "⚠️ Performance might need optimization ({}μs per event)",
            avg_event_time
        );
    }
