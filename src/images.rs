...
impl ImageManager {
    ...
    pub async fn load_image_from_base64(&self, data: &str, mime_type: Option<&str>) -> Result<String> {
    ...
    fn encode_sixel(&self, img: &DynamicImage) -> Result<String> {
        ...
        for y in (0..height).step_by(6) {
            for x in 0..width {
                ...
                let color_index = ((pixel[0] as u16 + pixel[1] as u16 + pixel[2] as u16) / 3 / 16) % 16;
