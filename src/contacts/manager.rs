use crate::contacts::{
    ContactsError, ContactsResult, Contact, ContactSearchCriteria, 
...
impl ContactsManager {
    ...
    pub async fn new(database: ContactsDatabase, token_manager: TokenManager) -> ContactsResult<Self> {
    ...
    pub async fn sync_account_contacts(&self, account_id: &str, provider_type: &str) -> ContactsResult<SyncSummary> {
        let provider = self.providers.get(provider_type)
            .ok_or_else(|| ContactsError::SyncError(format!("Unknown provider: {}", provider_type)))?;
        ...
        for remote_contact in remote_contacts {
            ...
            match self.process_contact_sync(remote_contact, account_id, provider_type).await {
                ...
                Err(e) => {
                    summary.errors.push(format!("Contact {}: {}", contact_id, e));
                }
            }
        }
    ...
    pub async fn search_by_email(&self, email_query: &str, limit: Option<usize>) -> ContactsResult<Vec<Contact>> {
    ...
    pub async fn get_account_contacts(&self, account_id: &str, provider_type: &str) -> ContactsResult<Vec<Contact>> {
        let source = match provider_type {
            "google" => ContactSource::Google { account_id: account_id.to_string() },
            "outlook" => ContactSource::Outlook { account_id: account_id.to_string() },
            _ => return Err(ContactsError::InvalidData(format!("Unknown provider: {}", provider_type))),
        };
    ...
    async fn process_contact_sync(
        ...
        let existing_contact = existing_contacts.iter()
    ...
    fn get_provider_for_source(&self, source: &ContactSource) -> Option<&Box<dyn ContactsProvider>> {
        match source {
...
mod tests {
    ...
    async fn test_needs_update() {
        ...
        let mut contact1 = Contact::new(
            "test-id".to_string(),
            ContactSource::Google { account_id: "test@gmail.com".to_string() },
            "John Doe".to_string(),
        );
        ...
        contact2.emails.push(ContactEmail::new("john@example.com".to_string(), "work".to_string()));
        assert!(manager.needs_update(&contact1, &contact2));
