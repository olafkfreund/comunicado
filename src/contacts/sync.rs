use crate::contacts::{ContactSource, ContactsError, ContactsManager, ContactsResult};
use std::sync::Arc;
...
impl ContactsSyncEngine {
    ...
    pub async fn start(&mut self) -> ContactsResult<mpsc::UnboundedReceiver<SyncProgress>> {
        ...
        if *is_running {
            return Err(ContactsError::SyncError(
...
impl SyncProgress {
    ...
    pub fn message(&self) -> String {
        match self {
            ...
            SyncProgress::ManualSyncStarted { .. } => {
                "Starting manual contacts sync...".to_string()
            }
            ...
            SyncProgress::AccountSyncStarted {
                account_id,
                provider,
                ..
            } => {
            ...
            SyncProgress::AccountSyncCompleted {
                account_id,
                summary,
                ..
            } => {
            ...
            SyncProgress::AccountSyncFailed {
                account_id, error, ..
            } => {
    ...
    pub fn is_error(&self) -> bool {
        matches!(
            self,
            SyncProgress::SyncFailed { .. } | SyncProgress::AccountSyncFailed { .. }
        )
    }
