...
use crate::email::{EmailDatabase, DatabaseStats};
use crate::oauth2::{TokenManager, AccountConfig, SecureStorage};
use crate::imap::ImapAccountManager;
use crate::keyboard::{KeyboardManager, KeyboardConfig, KeyboardShortcut, KeyboardAction};
...
impl CliHandler {
    ...
    async fn handle_test(&self, args: TestArgs, _dry_run: bool) -> Result<()> {
        ...
        let accounts_to_test = if let Some(account_name) = &args.account {
            ...
            if let Some(account) = accounts.iter().find(|a| &a.display_name == account_name || &a.email_address == account_name) {
                vec![account.clone()]
            } else {
            ...
            if accounts.len() == 1 {
                ...
                println!("Multiple accounts found. Use --all to test all, or specify --account <name>");
                for (i, account) in accounts.iter().enumerate() {
                    println!("  {}: {} ({})", i + 1, account.display_name, account.email_address);
                }
                return Ok(());
            }
        };
    ...
    async fn test_account(&self, account: &AccountConfig, args: &TestArgs) -> Result<bool> {
        println!("🔍 Testing account: {} ({})", account.display_name, account.email_address);
        ...
        match &account.auth_type {
            crate::oauth2::AuthType::OAuth2 => {
                if let Some(ref token_manager) = self.token_manager {
                    ...
                    match token_manager.get_valid_access_token(&account.email_address).await? {
                        Some(_token) => Ok(()),
    ...
    async fn test_imap_operations(&self, account: &AccountConfig) -> Result<()> {
        ...
        let inbox_status = client.get_folder_status("INBOX", &["MESSAGES", "UNSEEN"]).await?;
        
        tracing::debug!("INBOX status: {} messages", inbox_status.exists.unwrap_or(0));
    ...
    async fn handle_import(&self, args: ImportArgs, dry_run: bool) -> Result<()> {
        ...
        if let Some(ref google_file) = args.google_credentials {
            println!("📧 Importing Google OAuth2 credentials from: {}", google_file.display());
            imported_count += self.import_google_credentials(google_file, args.force, dry_run).await?;
        }
        
        // Import Microsoft credentials
        if let Some(ref microsoft_file) = args.microsoft_credentials {
            println!("📧 Importing Microsoft OAuth2 credentials from: {}", microsoft_file.display());
            imported_count += self.import_microsoft_credentials(microsoft_file, args.force, dry_run).await?;
        }
        
        // Import generic account config
        if let Some(ref config_file) = args.account_config {
            println!("⚙️  Importing account configuration from: {}", config_file.display());
            imported_count += self.import_account_config(config_file, args.force, dry_run).await?;
        }
        
        // Import Thunderbird profile
        if let Some(ref profile_dir) = args.thunderbird_profile {
            println!("🦅 Importing Thunderbird profile from: {}", profile_dir.display());
            imported_count += self.import_thunderbird_profile(profile_dir, args.force, dry_run).await?;
        }
    ...
    async fn import_google_credentials(&self, file_path: &PathBuf, _force: bool, dry_run: bool) -> Result<u32> {
    ...
    async fn import_microsoft_credentials(&self, file_path: &PathBuf, _force: bool, dry_run: bool) -> Result<u32> {
        let content = std::fs::read_to_string(file_path)?;
        let credentials: serde_json::Value = serde_json::from_str(&content)?;
        ...
        if !dry_run {
            ...
            self.storage.store_oauth_config("microsoft", &oauth_config)?;
    ...
    async fn import_account_config(&self, _file_path: &PathBuf, _force: bool, _dry_run: bool) -> Result<u32> {
    ...
    /// Import Thunderbird profile
    async fn import_thunderbird_profile(&self, _profile_dir: &PathBuf, _force: bool, _dry_run: bool) -> Result<u32> {
    ...
    async fn handle_troubleshoot(&self, args: TroubleshootArgs) -> Result<()> {
        ...
        if issues_found == 0 {
            ...
            println!("⚠️  Found {} potential issue(s) - see details above", issues_found);
        }
    ...
    async fn troubleshoot_authentication(&self) -> Result<u32> {
        ...
        for account in accounts {
            print!("   Checking {} ({})... ", account.display_name, account.email_address);
    ...
    async fn handle_database(&self, args: DatabaseArgs, dry_run: bool) -> Result<()> {
        match args.command {
            ...
            DatabaseCommands::Clean { account, aggressive, older_than } => {
                self.handle_database_clean(account, aggressive, older_than, dry_run).await
            }
            DatabaseCommands::Rebuild { account, search_index } => {
                self.handle_database_rebuild(account, search_index, dry_run).await
            }
    ...
    async fn handle_database_check(&self, account: Option<String>, fix: bool, dry_run: bool) -> Result<()> {
    ...
    async fn handle_database_clean(&self, _account: Option<String>, _aggressive: bool, _older_than: Option<u32>, dry_run: bool) -> Result<()> {
        ...
        if !dry_run {
            ...
            println!("\n🗑️  Cleanup completed:");
            println!("   - Removed {} orphaned attachments", cleaned.orphaned_attachments);
            println!("   - Removed {} duplicate messages", cleaned.duplicate_messages);
    ...
    async fn handle_database_rebuild(&self, _account: Option<String>, search_index: bool, dry_run: bool) -> Result<()> {
    ...
    async fn handle_database_stats(&self, account: Option<String>, detailed: bool) -> Result<()> {
        ...
        if detailed {
            println!("\n📋 Detailed Statistics:");
            println!("   Database file size: {} MB", stats.db_size_bytes / (1024 * 1024));
    ...
    async fn handle_database_backup(&self, output: PathBuf, compress: bool, dry_run: bool) -> Result<()> {
    ...
    async fn handle_database_restore(&self, input: PathBuf, force: bool, dry_run: bool) -> Result<()> {
    ...
    async fn handle_keyboard(&self, args: KeyboardArgs, dry_run: bool) -> Result<()> {
        match args.command {
            ...
            KeyboardCommands::Remove { key } => {
                self.handle_keyboard_remove(key, dry_run).await
            }
            ...
            KeyboardCommands::Validate { config } => {
                self.handle_keyboard_validate(config).await
            }
        }
    }
        ...
        if let Some(ref cat) = category {
            ...
            if let Some(shortcuts) = categories.get(cat) {
                ...
                for (shortcut, action, description) in shortcuts {
                    if detailed {
                        println!("  {:20} {:30} {}", shortcut.to_string(), format!("{:?}", action), description);
                    } else {
    ...
    /// Set a keyboard shortcut
    async fn handle_keyboard_set(&self, action_str: String, key_str: String, force: bool, dry_run: bool) -> Result<()> {
        ...
        if keyboard_manager.config().has_conflict(&shortcut) && !force {
            ...
            println!("\n❌ Shortcut '{}' is already assigned to action: {:?}", shortcut, existing_action);
        ...
        if !dry_run {
            ...
            println!("\n✅ Shortcut '{}' assigned to action: {:?}", shortcut, action);
        } else {
            println!("\n✅ Would assign shortcut '{}' to action: {:?}", shortcut, action);
        }
    ...
    /// Reset keyboard shortcuts to defaults
    async fn handle_keyboard_reset(&self, _category: Option<String>, force: bool, dry_run: bool) -> Result<()> {
    ...
    /// Export keyboard shortcuts to file
    async fn handle_keyboard_export(&self, output: PathBuf, format: String, dry_run: bool) -> Result<()> {
        ...
        match format.as_str() {
            ...
            "json" => {
                ...
                println!("✅ Keyboard shortcuts exported to: {} (JSON format)", output.display());
            }
            _ => {
                return Err(anyhow!("Unsupported export format: {}. Use 'toml' or 'json'", format));
            }
        }
    ...
    /// Import keyboard shortcuts from file
    async fn handle_keyboard_import(&self, input: PathBuf, _merge: bool, dry_run: bool) -> Result<()> {
            ...
            key if key.starts_with('f') && key.len() > 1 => {
                ...
                if let Ok(num) = num_str.parse::<u8>() {
                    if num >= 1 && num <= 12 {
                        ...
                        return Err(anyhow!("Function key number must be between 1 and 12: {}", key));
                    }
                } else {
