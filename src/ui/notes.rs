...
impl NotesUI {
    ...
    pub async fn convert_email_to_note(
        &self,
        email: &crate::email::EmailMessage,
        ...
        if let Some(ref conversion_service) = self.conversion_service {
            ...
            let note = conversion_service
                .convert_email_to_note(email, directory_id)
                .await?;
    ...
    pub async fn convert_event_to_note(
        &self,
        ...
        if let Some(ref conversion_service) = self.conversion_service {
            ...
            let note = conversion_service
                .convert_event_to_note(event, directory_id)
                .await?;
    ...
    pub async fn convert_kde_message_to_note(
        &self,
        title: &str,
        content: &str,
    ) -> Result<Note, Box<dyn std::error::Error + Send + Sync>> {
        if let Some(ref conversion_service) = self.conversion_service {
            ...
            // Convert KDE Connect message to note
            let note = conversion_service
                .convert_kde_message_to_note(title, content, directory_id)
                .await?;
    ...
    pub async fn search_notes(
        ...
        Vec<crate::plugins::notes::types::NoteSearchResult>,
