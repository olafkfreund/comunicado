...
impl AccountManagerUI {
    ...
    pub fn get_account_data(
        &self,
    ) -> Option<(String, String, String, String, u16, bool, String, String)> {
        ...
        match (&self.mode, key) {
            ...
            (AccountManagerMode::Add | AccountManagerMode::Edit(_), KeyCode::Tab) => {
                if self.editing_field.is_none() {
                    ...
                    let max_fields = if self.selected_account_type == AccountType::Custom {
                        6
                    } else {
                        2
                    };
    ...
    pub fn render(&mut self, frame: &mut Frame, area: Rect, theme: &Theme) {
        match self.mode.clone() {
            ...
            AccountManagerMode::Delete(account_id) => {
                self.render_delete_confirmation(frame, area, theme, &account_id)
            }
    ...
    fn render_add_account(&mut self, frame: &mut Frame, area: Rect, _theme: &Theme) {
        ...
        form_lines.push(format!(
            "Account Type: {}",
            self.selected_account_type.as_str()
        ));
        ...
        if self.selected_account_type == AccountType::Custom {
            ...
            form_lines.push(format!(
                "  Use SSL/TLS: {}",
                if self.form_use_ssl { "Yes" } else { "No" }
            ));
        }
    ...
    fn render_connection_test(&mut self, frame: &mut Frame, area: Rect, _theme: &Theme) {
        ...
        let (title, message, color) = match &self.connection_status {
            ConnectionStatus::Idle => (
                "Connection Test",
                "Preparing to test connection...".to_string(),
                Color::Gray,
            ),
            ConnectionStatus::Testing => (
                "Testing Connection",
                "Connecting to email server...".to_string(),
                Color::Yellow,
            ),
            ConnectionStatus::Success(msg) => {
                ("✅ Connection Successful", msg.clone(), Color::Green)
            }
