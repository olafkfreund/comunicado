...
use crate::events::types::{
    UIEvent, UIEventData, EmailEvent, EmailEventData, CalendarEvent, CalendarEventData, AccountEvent, AccountEventData,
    FocusedPane, UIMode, KeyEventData
};
...
impl EventDrivenUIIntegration {
    ...
    fn initialize_default_keybindings(&mut self) {
        // Email operations
        self.key_bindings.insert("d".to_string(), UIAction::DeleteCurrentEmail);
        self.key_bindings.insert("a".to_string(), UIAction::ArchiveCurrentEmail);
        self.key_bindings.insert("r".to_string(), UIAction::ReplyToCurrentEmail);
        self.key_bindings.insert("f".to_string(), UIAction::ForwardCurrentEmail);
        self.key_bindings.insert("c".to_string(), UIAction::ComposeNewEmail);
        self.key_bindings.insert("m".to_string(), UIAction::MarkCurrentEmailRead);
        self.key_bindings.insert("u".to_string(), UIAction::MarkCurrentEmailUnread);
        
        // Calendar operations
        self.key_bindings.insert("n".to_string(), UIAction::CreateCalendarEvent);
        self.key_bindings.insert("e".to_string(), UIAction::EditCurrentEvent);
        self.key_bindings.insert("Delete".to_string(), UIAction::DeleteCurrentEvent);
        self.key_bindings.insert("Enter".to_string(), UIAction::ViewCurrentEvent);
        
        // Account operations
        self.key_bindings.insert("s".to_string(), UIAction::SyncCurrentAccount);
        self.key_bindings.insert("Ctrl+a".to_string(), UIAction::AddNewAccount);
        
        // Navigation
        self.key_bindings.insert("j".to_string(), UIAction::NextItem);
        self.key_bindings.insert("k".to_string(), UIAction::PreviousItem);
        self.key_bindings.insert("1".to_string(), UIAction::SwitchPane(FocusedPane::AccountList));
        self.key_bindings.insert("2".to_string(), UIAction::SwitchPane(FocusedPane::FolderTree));
        self.key_bindings.insert("3".to_string(), UIAction::SwitchPane(FocusedPane::MessageList));
        self.key_bindings.insert("4".to_string(), UIAction::SwitchPane(FocusedPane::ContentPreview));
        
        // Search
        self.key_bindings.insert("/".to_string(), UIAction::StartSearch);
        self.key_bindings.insert("Escape".to_string(), UIAction::ClearSearch);
        
        // System
        self.key_bindings.insert("?".to_string(), UIAction::ShowHelp);
    ...
    pub async fn handle_key_event(&mut self, key_data: KeyEventData) -> Result<bool, EventError> {
        ...
        if let Some(action) = self.key_bindings.get(&key_string).cloned() {
            tracing::debug!("Processing UI action: {:?} from key: {}", action, key_string);
    ...
    async fn execute_ui_action(&mut self, action: UIAction) -> Result<(), EventError> {
        match action {
            ...
            UIAction::DeleteCurrentEmail => {
                self.execute_email_action(EmailAction::DeleteCurrentEmail).await?;
            }
            UIAction::ArchiveCurrentEmail => {
                self.execute_email_action(EmailAction::ArchiveCurrentEmail).await?;
            }
            UIAction::MarkCurrentEmailRead => {
                self.execute_email_action(EmailAction::MarkCurrentEmailAsRead).await?;
            }
            UIAction::MarkCurrentEmailUnread => {
                self.execute_email_action(EmailAction::MarkCurrentEmailAsUnread).await?;
            }
            UIAction::ReplyToCurrentEmail => {
                self.execute_email_action(EmailAction::ReplyToCurrentEmail).await?;
            }
            UIAction::ForwardCurrentEmail => {
                self.execute_email_action(EmailAction::ForwardCurrentEmail).await?;
            }
            UIAction::ComposeNewEmail => {
                self.execute_email_action(EmailAction::ComposeNewEmail).await?;
            }
            ...
            UIAction::EditCurrentEvent => {
                if let (Some(calendar_id), Some(event_id)) = (&self.selected_calendar_id, &self.selected_email_id) {
    ...
    async fn execute_email_action(&self, action: EmailAction) -> Result<(), EventError> {
        ...
        match action {
            EmailAction::DeleteCurrentEmail => {
                if let (Some(account_id), Some(email_id)) = (&self.selected_account_id, &self.selected_email_id) {
