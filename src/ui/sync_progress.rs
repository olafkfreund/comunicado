...
use crate::email::sync_engine::{SyncProgress, SyncPhase};
...
impl SyncProgressOverlay {
    ...
    pub fn cleanup_completed(&mut self, threshold: ChronoDuration) {
        let now = Utc::now();
        let keys_to_remove: Vec<String> = self.active_syncs
            .iter()
            .filter(|(_, progress)| {
                match progress.phase {
                    SyncPhase::Complete | SyncPhase::Error(_) => {
    ...
    fn render_progress_gauge(&self, frame: &mut Frame, area: Rect, progress: &SyncProgress, theme: &Theme) {
        let progress_percent = if progress.total_messages > 0 {
    ...
    fn render_sync_stats(&self, frame: &mut Frame, area: Rect, progress: &SyncProgress, theme: &Theme) {
        ...
        let stats_text = vec![
            ...
            Line::from(vec![
                Span::styled("Downloaded: ", Style::default().add_modifier(Modifier::BOLD)),
                Span::raw(bytes_str),
            ]),
        ];
    ...
    fn render_phase_details(&self, frame: &mut Frame, area: Rect, progress: &SyncProgress, theme: &Theme) {
        let phase_text = match &progress.phase {
            ...
            SyncPhase::Complete => vec![
                ...
                Line::from(format!("• {} messages processed", progress.messages_processed)),
                Line::from(format!("• {} bytes downloaded", format_bytes(progress.bytes_downloaded))),
            ...
            SyncPhase::Error(error) => vec![
                ...
                Line::from(""),
                Line::from(Span::styled("Error details:", Style::default().add_modifier(Modifier::BOLD))),
                Line::from(error.clone()),
            ],
        };
        
        let phase_details = Paragraph::new(phase_text)
            .block(
                Block::default()
                    .title("Current Phase")
                    .borders(Borders::ALL)
