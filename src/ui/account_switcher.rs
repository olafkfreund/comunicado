...
impl AccountSwitcher {
    ...
    pub fn update_account_status(
            ...
            .find(|a| a.account_id == account_id)
        {
            ...
            account.is_online = !matches!(
                status,
                AccountSyncStatus::Offline | AccountSyncStatus::Error
            );
    ...
    pub fn render(
        ...
        let (items, title, should_use_state) = if self.is_expanded {
            ...
            for (index, account) in self.accounts.iter().enumerate() {
                let item = AccountSwitcher::create_account_list_item_static(
                    account,
            ...
            (
                items,
                format!("Accounts ({}/{})", self.accounts.len(), self.accounts.len()),
                true,
            )
        } else {
            // Show only current account when collapsed
            if let Some(current_account) = self.get_current_account() {
                let item = AccountSwitcher::create_current_account_item_static(
                    current_account,
                    &self.accounts,
                    theme,
                );
    ...
    fn create_account_list_item_static(
        account: &AccountItem,
        is_selected: bool,
        theme: &Theme,
    ) -> ListItem<'static> {
    ...
    fn create_current_account_item_static(
        account: &AccountItem,
