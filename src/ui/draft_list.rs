...
use ratatui::{
    ...
    text::{Line, Text},
    widgets::{
        Block, Borders, List, ListItem, ListState, Paragraph, Clear
    },
    Frame,
};
use chrono::Local;
...
impl DraftListUI {
    ...
    pub fn update_drafts(&mut self, mut drafts: Vec<StoredDraft>) {
        ...
        if self.selected_index >= self.drafts.len() {
            self.selected_index = if self.drafts.is_empty() { 0 } else { self.drafts.len() - 1 };
        }
    ...
    fn render_draft_list(&mut self, f: &mut Frame, area: Rect, theme: &Theme) {
        ...
        for (i, draft) in self.drafts.iter().enumerate() {
            ...
            items.push(Self::create_draft_list_item_static(draft, is_selected, theme));
        }
        
        let list = List::new(items)
            .block(Block::default()
                .title(format!("Drafts ({}) - Sort: {:?} {}", 
                    drafts_len, 
                    sort_by,
    ...
    fn create_draft_list_item_static<'a>(draft: &'a StoredDraft, is_selected: bool, theme: &'a Theme) -> ListItem<'a> {
        // Get recipients
        let to_display = draft.to_addrs.first()
            .map(|r| if r.len() > 25 { format!("{}...", &r[..22]) } else { r.clone() })
            .unwrap_or_else(|| "No recipient".to_string());
        
        // Format dates
        let updated_time = draft.updated_at
            .with_timezone(&Local)
            .format("%m/%d %H:%M")
            .to_string();
    ...
    fn render_draft_details(&self, f: &mut Frame, area: Rect, theme: &Theme) {
        if let Some(draft) = self.get_selected_draft() {
            ...
            // Format dates
            let created_time = draft.created_at
                .with_timezone(&Local)
                .format("%Y-%m-%d %H:%M:%S")
                .to_string();
            let updated_time = draft.updated_at
            ...
            lines.extend(vec![
                Line::from(""),
                Line::from(format!("Subject: {}", if draft.subject.is_empty() { "(No subject)" } else { &draft.subject })),
                Line::from(""),
                Line::from(format!("Created: {}", created_time)),
                Line::from(format!("Updated: {}", updated_time)),
                Line::from(format!("Auto-saved: {}", if draft.auto_saved { "Yes" } else { "No" })),
                Line::from(""),
