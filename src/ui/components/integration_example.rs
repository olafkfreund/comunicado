...
use super::{AppMode, EventResult, ModularUI, ModularUIMetrics, UIEvent};
use crate::{
...
mod example_usage {
    ...
    async fn test_modular_ui_integration() {
        ...
        // Test event handling
        let key_event =
            crossterm::event::Event::Key(KeyEvent::new(KeyCode::F(3), KeyModifiers::NONE));
    ...
    async fn test_component_lifecycle() {
        ...
        let key_event =
            crossterm::event::Event::Key(KeyEvent::new(KeyCode::Char('j'), KeyModifiers::NONE));
        app.handle_event(key_event).unwrap();
...
pub mod performance_demo {
    ...
    impl PerformanceResults {
        ...
        pub fn display(&self) {
            ...
            println!(
                "   Initialization: {:.2}ms",
                self.initialization_time.as_secs_f64() * 1000.0
            );
            println!(
                "   Avg frame time: {:.2}ms",
            ...
            let improvement_percent = ((estimated_monolithic_frame_time
                - self.avg_frame_time.as_secs_f64())
                / estimated_monolithic_frame_time)
                * 100.0;

            println!(
                "   Estimated improvement over monolithic: {:.1}%",
                improvement_percent
            );
        }
    }
...
async fn example_main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    loop {
        // Render the UI
        terminal.draw(|f| {
            if let Err(e) = app.render(f, f.area()) {
