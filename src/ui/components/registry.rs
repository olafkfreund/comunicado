...
impl ComponentRegistry {
    ...
    pub fn register<T: UIComponent + 'static>(
        &mut self,
        component: T,
        ...
        if let Some(new_focus) = component_id {
            if let Some(handle) = self.components.get_mut(&new_focus) {
                if handle.can_focus() {
                    ...
                    return Err(ComponentError::NotReady {
                        state: handle.state(),
                    });
                }
            } else {
                return Err(ComponentError::Unknown(format!(
                    "Component {:?} not found",
                    new_focus
                )));
            }
    ...
    pub fn cycle_focus(&mut self) -> ComponentResult<()> {
        let focusable_components: Vec<ComponentId> = self
            .components
            .iter()
...
mod tests {
    use super::*;
    use crate::ui::components::traits::{EventResult, RenderContext, UIComponent, UIEvent};
