...
impl CalendarComponent {
    ...
    fn handle_calendar_action(&mut self, action: CalendarAction) -> ComponentResult<EventResult> {
        match action {
            ...
            CalendarAction::SelectEvent(event_id) => {
                ...
                self.selected_event = self.events.iter()
    ...
    fn render_header(&self, context: &mut RenderContext<'_>, area: Rect) -> ComponentResult<()> {
        ...
        let date_text = match self.current_view {
            ...
            CalendarViewMode::Week => {
                let week_start = self.current_date - Duration::days(self.current_date.weekday().num_days_from_monday() as i64);
    ...
    /// Render month view
    fn render_month_view(&self, context: &mut RenderContext<'_>, area: Rect) -> ComponentResult<()> {
    ...
    fn render_day_view(&self, context: &mut RenderContext<'_>, area: Rect) -> ComponentResult<()> {
        ...
        let content = Paragraph::new("Day view placeholder - showing hourly schedule for selected day")
            .block(day_block)
            .alignment(Alignment::Center);
    ...
    /// Render agenda view
    fn render_agenda_view(&self, context: &mut RenderContext<'_>, area: Rect) -> ComponentResult<()> {
        ...
        let event_items: Vec<ListItem> = self.events.iter()
            .filter(|event| {
    ...
    /// Render status line
    fn render_status_line(&self, context: &mut RenderContext<'_>, area: Rect) -> ComponentResult<()> {
    ...
    fn handle_key_event(&mut self, key: KeyEvent) -> ComponentResult<EventResult> {
        match key.code {
            KeyCode::Char('1') => self.handle_calendar_action(CalendarAction::ChangeView(CalendarViewMode::Month)),
            KeyCode::Char('2') => self.handle_calendar_action(CalendarAction::ChangeView(CalendarViewMode::Week)),
            KeyCode::Char('3') => self.handle_calendar_action(CalendarAction::ChangeView(CalendarViewMode::Day)),
            KeyCode::Char('4') => self.handle_calendar_action(CalendarAction::ChangeView(CalendarViewMode::Agenda)),
            KeyCode::Char('t') | KeyCode::Char('T') => self.handle_calendar_action(CalendarAction::Today),
            KeyCode::Left | KeyCode::Char('h') => self.handle_calendar_action(CalendarAction::PreviousPeriod),
            KeyCode::Right | KeyCode::Char('l') => self.handle_calendar_action(CalendarAction::NextPeriod),
