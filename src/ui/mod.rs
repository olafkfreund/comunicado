...
impl UI {
    ...
    pub fn render_with_startup_progress(
        &mut self,
        frame: &mut Frame,
        keyboard_manager: &KeyboardManager,
        startup_manager: Option<&crate::startup::StartupProgressManager>,
    ) {
        let size = frame.size();

        match self.mode {
            UIMode::StartPage => {
                // Render start page with startup progress support
                let theme = self.theme_manager.current_theme();
                self.start_page.render_with_startup_manager(frame, size, theme, startup_manager);
            }
            _ => {
                // For other modes, use the regular render method
                self.render_with_keyboard_manager(frame, keyboard_manager);
                return;
            }
        }

        // Update status bar and notifications for start page mode
        ...
        self.status_bar.render(frame, status_rect, self.theme_manager.current_theme());

        // Handle sync progress overlay (if any)
        if self.sync_progress_overlay.is_visible() {
            // In start page mode, sync progress overlay might not be needed
            ...
            self.sync_progress_overlay.render(frame, popup_area, self.theme_manager.current_theme());
        }
        ...
        if let Some(ref message) = self.notification_message {
            if let Some(expires_at) = self.notification_expires_at {
                if tokio::time::Instant::now() < expires_at {
                    // Show notification (simplified for start page)
                    ...
                    let notification_block = ratatui::widgets::Block::default()
                        .title("Notification")
                        .borders(ratatui::widgets::Borders::ALL)
                        .border_style(ratatui::style::Style::default().fg(ratatui::style::Color::Yellow));
                    
                    let notification_paragraph = ratatui::widgets::Paragraph::new(message.as_str())
