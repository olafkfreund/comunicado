...
impl MouseEventProcessor {
    ...
    pub fn update_component_areas(&mut self, areas: std::collections::HashMap<UIComponent, Rect>) {
        ...
        tracing::trace!(
            "Updated component areas: {} components",
            self.component_areas.len()
        );
    }
    ...
    pub async fn process_mouse_event(&mut self, mouse_event: MouseEvent) -> Result<MouseAction> {
        ...
        if mouse_event.column >= self.terminal_size.0 || mouse_event.row >= self.terminal_size.1 {
            tracing::debug!(
                "Mouse coordinates out of bounds: ({}, {}) vs terminal ({}, {})",
                mouse_event.column,
                mouse_event.row,
        ...
        let relative_coords =
            self.get_relative_coordinates(&component, mouse_event.column, mouse_event.row);
        ...
        let action = match mouse_event.kind {
            ...
            MouseEventKind::Up(button) => {
                let action = self.handle_mouse_up(
                    button,
                    component,
                    mouse_event.column,
                    mouse_event.row,
            ...
            MouseEventKind::Drag(button) => self.handle_mouse_drag(
                button,
                component,
                mouse_event.column,
                mouse_event.row,
            ...
            MouseEventKind::ScrollDown => self.handle_mouse_scroll(
                component,
                mouse_event.column,
                mouse_event.row,
                relative_coords,
            ...
            MouseEventKind::ScrollUp => self.handle_mouse_scroll(
                component,
                mouse_event.column,
                mouse_event.row,
                relative_coords,
    ...
    fn handle_mouse_down(
        ...
        x: u16,
        y: u16,
        relative_coords: Option<(u16, u16)>,
    ) -> MouseAction {
        match button {
            MouseButton::Left => match component {
                UIComponent::MessageList => {
                    if let Some((rel_x, rel_y)) = relative_coords {
                        MouseAction::SelectMessage {
                            row: rel_y,
                            column: rel_x,
                        }
                    } else {
                ...
                UIComponent::Calendar => {
                    if let Some((rel_x, rel_y)) = relative_coords {
                        MouseAction::SelectCalendarDate {
                            row: rel_y,
                            column: rel_x,
                        }
                    } else {
            ...
            MouseButton::Right => match component {
                UIComponent::MessageList => {
                    if let Some((rel_x, rel_y)) = relative_coords {
            ...
            MouseButton::Middle => match component {
    ...
    fn handle_mouse_up(
        ...
        _x: u16,
        _y: u16,
        _relative_coords: Option<(u16, u16)>,
    ) -> MouseAction {
    ...
    fn handle_mouse_move(
        ...
        _y: u16,
        relative_coords: Option<(u16, u16)>,
        _old_hovered: Option<UIComponent>,
    ) -> MouseAction {
        match component {
            UIComponent::MessageList => {
                if let Some((rel_x, rel_y)) = relative_coords {
                    MouseAction::HoverMessage {
                        row: rel_y,
                        column: rel_x,
                    }
                } else {
...
mod tests {
    ...
    fn test_coordinate_mapping() {
        ...
        assert_eq!(
            processor.get_component_at_coordinates(25, 10),
            UIComponent::MessageList
        );
        assert_eq!(
            processor.get_component_at_coordinates(60, 10),
            UIComponent::FolderTree
        );
        assert_eq!(
            processor.get_component_at_coordinates(90, 10),
            UIComponent::None
        );
    }
    ...
    fn test_relative_coordinates() {
        ...
        assert_eq!(
            processor.get_relative_coordinates(&UIComponent::MessageList, 15, 10),
            Some((5, 5))
        );
        assert_eq!(
            processor.get_relative_coordinates(&UIComponent::MessageList, 5, 5),
            None
        );
    }
}
