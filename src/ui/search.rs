...
impl SearchUI {
    ...
    pub fn new() -> Self {
        let available_modes = vec![
            (
                SearchMode::FullText,
                "Full Text".to_string(),
                "Search across all email content".to_string(),
            ),
            (
                SearchMode::Subject,
                "Subject".to_string(),
                "Search only in subject lines".to_string(),
            ),
            (
                SearchMode::From,
                "From".to_string(),
                "Search only in sender information".to_string(),
            ),
            (
                SearchMode::Body,
                "Body".to_string(),
                "Search only in email body content".to_string(),
            ),
            (
                SearchMode::Advanced,
                "Advanced".to_string(),
                "Advanced search with multiple criteria".to_string(),
            ),
        ];
    ...
    pub fn handle_key(&mut self, key: crossterm::event::KeyCode) -> Option<SearchAction> {
        ...
        match key {
            ...
            crossterm::event::KeyCode::Enter => Some(SearchAction::SelectResult),
            ...
            crossterm::event::KeyCode::Tab => Some(SearchAction::ToggleModeSelector),
            crossterm::event::KeyCode::Esc => Some(SearchAction::ClearSearch),
    ...
    fn render_status(&self, frame: &mut Frame, area: Rect, theme: &Theme) {
        let status_text = if let Some(ref error) = self.error_message {
            ...
            format!(
                "Found {} results in {}ms",
                self.total_results, self._search_time_ms
            )
        } else {
    ...
    fn render_mode_selector(&self, frame: &mut Frame, area: Rect, theme: &Theme) {
        ...
        let mode_list = List::new(items).block(
            Block::default()
                .title(" Search Mode ")
                .borders(Borders::ALL)
                .border_style(Style::default().fg(theme.colors.palette.accent)),
...
impl SearchEngine {
    ...
    pub async fn search(
        ...
        let messages = self
            .database
            .search_messages(account_id, &fts_query, limit)
            .await?;
    ...
    fn build_fts_query(
        ...
        let fts_query = match mode {
            ...
            SearchMode::From => format!(
                "(from_addr:{} OR from_name:{})",
    ...
    async fn create_search_result(
        ...
        if self.field_matches(&message.from_addr, &query_terms)
            || message
                .from_name
                .as_ref()
                .map(|n| self.field_matches(n, &query_terms))
                .unwrap_or(false)
        {
            matched_fields.push("from".to_string());
            let from_text = message
                .from_name
                .as_ref()
    ...
    fn create_snippet(
        ...
        let adjusted_highlights: Vec<(usize, usize)> = highlights
            .iter()
            .filter_map(|&(start, end)| {
                if start >= snippet_start && start < snippet_end {
                    Some((
                        start - snippet_start,
                        std::cmp::min(end, snippet_end) - snippet_start,
                    ))
                } else {
                    None
                }
