...
impl FormState {
    ...
    pub fn update_form_validity(&mut self) {
        self.form_is_valid = self.fields
            .values()
    ...
    pub fn valid_field_count(&self) -> usize {
        self.fields.values().filter(|field| field.is_valid()).count()
    }
    
    /// Count fields with errors
    pub fn error_field_count(&self) -> usize {
        self.fields.values().filter(|field| field.has_error()).count()
    }
    
    /// Count fields with warnings
    pub fn warning_field_count(&self) -> usize {
        self.fields.values().filter(|field| field.has_warning()).count()
    }
    ...
    pub fn completion_percentage(&self) -> f32 {
        ...
        let completed_fields = self.fields
            .values()
