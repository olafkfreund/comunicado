...
static URL_REGEX: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"^https?://[^\s/$.?#].[^\s]*$").expect("Invalid URL regex"));
        ...
        if !value.is_empty() && !EMAIL_REGEX.is_match(value) {
            Some(
                ValidationMessage::new("Invalid email format")
                    .with_suggestion(
                        "Enter a valid email address (e.g., user@example.com)".to_string(),
                    )
...
mod tests {
    ...
    fn test_email_validation() {
        ...
        let mut field = ValidatedField::new(vec![ValidationRule::new(
            "email",
            ValidationFunction::Email,
        )]);
