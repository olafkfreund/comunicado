...
impl AnimationDecoder {
    ...
    pub async fn decode_file(&self, path: &Path) -> AnimationResult<Animation> {
        let format = AnimationFormat::from_extension(path).ok_or_else(|| {
            AnimationError::UnsupportedFormat(
                path.extension()
                    .unwrap_or_default()
                    .to_string_lossy()
                    .to_string(),
        ...
        self.decode_bytes(&file_data, format, Some(path.to_path_buf()))
            .await
    }
        ...
        match gif::DecodeOptions::new().read_info(&mut cursor) {
            Ok(mut decoder) => {
                ...
                while let Some(frame) = decoder.read_next_frame().map_err(|e| {
                    ...
                    let img =
                        image::RgbImage::from_raw(width, height, rgb_data).ok_or_else(|| {
                            AnimationError::DecodeError(
                                "Failed to create RGB image from GIF frame".to_string(),
                            )
                        })?;
                ...
                let metadata = AnimationMetadata {
                    ...
                    background_color: decoder
                        .bg_color()
                        .map(|idx| [idx as u8, idx as u8, idx as u8, 255]),
                    created_at: Utc::now(),
...
impl Animation {
    ...
    pub fn update(&mut self) -> Option<&AnimationFrame> {
        ...
        let should_advance = if let Some(last_time) = self.last_frame_time {
            ...
            let current_frame_delay =
                Duration::from_millis(self.frames[self.current_frame].delay_ms as u64);
    ...
    pub async fn render_current_frame(
        ...
        // Check cache first
        if let Some((_, rendered_data)) = self
            .rendered_frames
            .iter()
        ...
        if let Some(frame) = self.current_frame_data() {
            let rendered = renderer
                .render_image(&frame.image, width, height)
                .await
                .map_err(|e| AnimationError::TerminalProtocol(e.to_string()))?;

            // Cache rendered frame
            self.rendered_frames
                .push_back((self.current_frame, rendered.clone()));
    ...
    pub async fn render_animation(
        ...
        if let Some(animation) = animations.iter_mut().find(|a| a.metadata.id == id) {
            ...
            if animation.current_frame_data().is_some() {
                let rendered = animation
                    .render_current_frame(&self.renderer, width, height)
                    .await?;
                Ok(Some(rendered))
            } else {
                Ok(None)
...
mod tests {
    ...
    fn test_animation_format_detection() {
        ...
        assert_eq!(AnimationFormat::from_extension(Path::new("test.jpg")), None);
    }
