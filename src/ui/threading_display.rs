...
impl ThreadingDisplay {
    ...
    pub fn get_threading_prefix(
        ...
        match self.style {
            ...
            ThreadingStyle::Lines => {
                self.get_lines_prefix(message, connection_type, thread_context)
            }
            ThreadingStyle::Modern => {
                self.get_modern_prefix(message, connection_type, thread_context)
            }
    ...
    fn get_minimal_prefix(&self, message: &MessageItem) -> ThreadingPrefix {
        ...
        ThreadingPrefix { spans, total_width }
    }
    ...
    fn get_lines_prefix(
        ...
        if message.thread_depth == 0 {
            ...
            if message.message_count > 1 {
                let symbol = if message.is_thread_expanded {
                    "─┐"
                } else {
                    "─► "
                };
    ...
    pub fn create_thread_header(
        ...
        if message.is_thread_root && message.message_count > 1 {
            // Thread indicator
            let thread_symbol = if message.is_thread_expanded {
                "📂"
            } else {
                "📁"
            };
...
mod tests {
    ...
    fn test_minimal_prefix() {
        ...
        let mut message =
            MessageItem::new("Test".to_string(), "sender".to_string(), "date".to_string());
