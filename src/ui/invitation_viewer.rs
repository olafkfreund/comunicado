...
use crate::calendar::invitation::{InvitationMethod, MeetingInvitation};
...
impl InvitationViewer {
        ...
        if let Some(ref invitation) = self.invitation {
            match invitation.method {
                    ...
                    if self.user_invited {
                        ...
                        match self.user_status {
                            Some(AttendeeStatus::NeedsAction) | None => {
                                self.actions
                                    .push((InvitationAction::Accept, "Accept".to_string()));
                                self.actions
                                    .push((InvitationAction::Tentative, "Maybe".to_string()));
                                self.actions
                                    .push((InvitationAction::Decline, "Decline".to_string()));
                            }
                            Some(AttendeeStatus::Accepted) => {
                                self.actions.push((
                                    InvitationAction::Tentative,
                                    "Change to Maybe".to_string(),
                                ));
                                self.actions.push((
                                    InvitationAction::Decline,
                                    "Change to Decline".to_string(),
                                ));
                            }
                            Some(AttendeeStatus::Tentative) => {
                                self.actions.push((
                                    InvitationAction::Accept,
                                    "Change to Accept".to_string(),
                                ));
                                self.actions.push((
                                    InvitationAction::Decline,
                                    "Change to Decline".to_string(),
                        ...
                        self.actions.push((
                            InvitationAction::AddToCalendar,
                ...
                InvitationMethod::Cancel => {
                    ...
                    self.actions
                        .push((InvitationAction::ViewDetails, "View Details".to_string()));
                }
            ...
            if !self.show_details {
                self.actions
                    .push((InvitationAction::ViewDetails, "Show Details".to_string()));
            }
            self.actions
                .push((InvitationAction::Close, "Close".to_string()));
        }
    }
    ...
    fn render_basic_info(
        ...
        let time_format = if invitation.all_day {
            format!(
                "🕐 {} (All day)",
                invitation.start_time.format("%A, %B %d, %Y")
            )
        } else {
            format!(
                "🕐 {} - {}",
    ...
    fn render_attendees(
        ...
        let list = List::new(items).style(Style::default().fg(theme.colors.palette.text_primary));
    ...
    fn render_actions(&self, frame: &mut Frame, area: Rect, theme: &Theme) {
        ...
        for (i, (_, label)) in self.actions.iter().enumerate() {
            ...
            let prefix = if i == self.selected_action {
                "► "
            } else {
                "  "
            };
