...
use crate::keyboard::{KeyboardManager, KeyboardAction, KeyboardShortcut};
...
impl KeyboardShortcutsUI {
    ...
    pub fn render(&mut self, frame: &mut Frame, area: Rect, theme: &Theme, keyboard_manager: &KeyboardManager) {
        ...
        for (category, category_shortcuts) in shortcuts {
            ...
            items.push(ListItem::new(Line::from(vec![
                Span::styled(format!(" {} ", category), theme.get_component_style("border", true)
            ...
            for (shortcut, action) in category_shortcuts {
                ...
                items.push(ListItem::new(Line::from(vec![
                    Span::styled(format!("  {:15}", shortcut_display), theme.get_component_style("button", true)
                        .add_modifier(Modifier::BOLD)),
        ...
        let visible_items = items.into_iter()
    ...
    fn organize_shortcuts(&self, keyboard_manager: &KeyboardManager) -> Vec<(&str, Vec<(KeyboardShortcut, KeyboardAction)>)> {
        ...
        for (shortcut, action) in keyboard_manager.get_all_shortcuts() {
            let category_index = match action {
                KeyboardAction::Quit | KeyboardAction::ForceQuit | KeyboardAction::ShowStartPage | KeyboardAction::ShowKeyboardShortcuts => 0,
                KeyboardAction::NextPane | KeyboardAction::PreviousPane | KeyboardAction::MoveUp | KeyboardAction::MoveDown | KeyboardAction::MoveLeft | KeyboardAction::MoveRight => 1,
                KeyboardAction::Select | KeyboardAction::Escape | KeyboardAction::ToggleExpanded => 2,
                KeyboardAction::VimMoveLeft | KeyboardAction::VimMoveRight | KeyboardAction::VimMoveUp | KeyboardAction::VimMoveDown => 8,
                KeyboardAction::ComposeEmail | KeyboardAction::ReplyEmail | KeyboardAction::ReplyAllEmail | KeyboardAction::ForwardEmail | KeyboardAction::DeleteEmail | KeyboardAction::MarkAsRead | KeyboardAction::MarkAsUnread => 4,
                KeyboardAction::AddAccount | KeyboardAction::RemoveAccount | KeyboardAction::RefreshAccount | KeyboardAction::SwitchAccount => 5,
                KeyboardAction::StartSearch | KeyboardAction::StartFolderSearch | KeyboardAction::EndSearch => 7,
                _ => 0, // Default to global actions
            };
            
            categories[category_index].1.push((shortcut.clone(), action.clone()));
        }
        
        // Filter out empty categories
        categories.into_iter().filter(|(_, shortcuts)| !shortcuts.is_empty()).collect()
    }
