...
use image::{codecs::gif::GifDecoder, DynamicImage, ImageOutputFormat};
...
impl AnimationManager {
    ...
    pub async fn load_gif_from_bytes(
        &self,
        data: &[u8],
        ...
        tracing::info!(
            "Loaded GIF animation: {} frames, {}ms duration",
            info.frame_count,
            info.duration_ms
        );
    ...
    pub async fn play_animation(
        ...
        animation_id: &str,
        loops: bool,
    ) -> Result<mpsc::UnboundedReceiver<AnimationResult>> {
            ...
            loop {
                ...
                // Update animation state
                {
                    ...
                    if let Some(state) = animations_guard.get_mut(&animation_id) {
                        state.current_frame =
                            (state.current_frame + 1) % animation.info.frame_count;
