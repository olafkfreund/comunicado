...
impl StartupProgressScreen {
    ...
    pub fn render(
        ...
        frame: &mut Frame,
        area: Rect,
        manager: &StartupProgressManager,
        theme: &Theme,
    ) {
    ...
    fn render_overall_progress(
        ...
        let stats = Paragraph::new(Line::from(stats_line)).alignment(Alignment::Center);

        frame.render_widget(stats, progress_chunks[1]);
    }

    fn render_current_phase_details(
        &self,
        frame: &mut Frame,
        area: Rect,
        manager: &StartupProgressManager,
        ...
        if let Some(current_phase) = manager.current_phase() {
            ...
            if current_phase.status().is_in_progress() && phase_progress > 0.0 {
                ...
                let status_text =
                    self.format_phase_status(current_phase.status(), &current_phase.timeout());
            ...
            if !logs.is_empty() {
                ...
                let logs_paragraph = Paragraph::new(log_lines).wrap(Wrap { trim: true });
    ...
    fn render_phase_list(
        ...
        let list = List::new(items).style(Style::default().fg(theme.colors.palette.text_secondary));
    ...
    fn render_error_summary(
        ...
        let block = Block::default()
            .title(title)
            .borders(Borders::ALL)
            .border_style(Style::default().fg(if critical_errors > 0 {
        ...
        let paragraph = Paragraph::new(error_text)
            .style(Style::default().fg(if critical_errors > 0 {
                theme.colors.palette.error
            } else {
                theme.colors.palette.warning
            }))
    ...
    fn get_status_color(
        &self,
        status: &crate::startup::progress::PhaseStatus,
        theme: &Theme,
    ) -> Color {
...
mod tests {
    ...
    fn test_status_color_mapping() {
        ...
        let timed_out = PhaseStatus::TimedOut {
            duration: Duration::from_secs(10),
        };

        assert_eq!(
            screen.get_status_color(&pending, &theme),
            theme.colors.palette.text_secondary
        );
        assert_eq!(
            screen.get_status_color(&in_progress, &theme),
            theme.colors.palette.accent
        );
        assert_eq!(
            screen.get_status_color(&completed, &theme),
            theme.colors.palette.success
        );
        assert_eq!(
            screen.get_status_color(&failed, &theme),
            theme.colors.palette.error
        );
        assert_eq!(
            screen.get_status_color(&timed_out, &theme),
            theme.colors.palette.warning
        );
    }
    ...
    fn test_format_phase_status() {
        ...
        let pending = PhaseStatus::Pending;
        assert_eq!(
            screen.format_phase_status(&pending, &timeout),
            "Waiting to start..."
        );
        ...
        assert_eq!(
            screen.format_phase_status(&completed, &timeout),
            "Completed in 5.0s"
        );
        ...
        assert_eq!(
            screen.format_phase_status(&failed, &timeout),
        ...
        assert_eq!(
            screen.format_phase_status(&timed_out, &timeout),
    ...
    fn test_render_with_errors() {
        ...
        manager
            .fail_phase("IMAP Manager", "Connection timeout".to_string())
            .unwrap();
