...
impl SuggestionManager {
    ...
    fn load_context_suggestions(&mut self) {
        let contexts = vec![
            ...
            ("database", vec!["data base", "DB", "datastore", "repository"]),
            ("username", vec!["user name", "login", "account", "user ID"]),
            ("password", vec!["passcode", "PIN", "credentials", "authentication"]),
            ("internet", vec!["web", "online", "net", "worldwide web"]),
            ("computer", vec!["PC", "machine", "system", "device"]),
            ("software", vec!["program", "application", "app", "tool"]),
            ("hardware", vec!["equipment", "device", "component", "system"]),
            ("network", vec!["connection", "link", "communication", "internet"]),
        ];
    ...
    pub fn generate_suggestions(&self, misspelled: &MisspelledWord, context: &str) -> SpellSuggestion {
        ...
        if let Some(correction) = self.common_typos.get(&misspelled.word.to_lowercase()) {
            suggestions.insert(0, SuggestionOption {
    ...
    /// Add context-based suggestions
    fn add_context_suggestions(&self, word: &str, context: &str, suggestions: &mut Vec<SuggestionOption>) {
        let word_lower = word.to_lowercase();
