...
use super::types::{KeyInfo, EncryptionConfig, TrustLevel};
...
impl EncryptionUI {
    ...
    async fn handle_keys_tab_key(&mut self, key: KeyEvent) -> Result<bool, Box<dyn std::error::Error>> {
        match key.code {
            KeyCode::Up => {
    ...
    async fn handle_config_tab_key(&mut self, _key: KeyEvent) -> Result<bool, Box<dyn std::error::Error>> {
        // TODO: Implement config editing
        Ok(false)
    }
    
    /// Handle import tab input
    async fn handle_import_tab_key(&mut self, key: KeyEvent) -> Result<bool, Box<dyn std::error::Error>> {
        match key.code {
            KeyCode::Char(c) => {
            ...
            KeyCode::Enter => {
                if !self.key_buffer.trim().is_empty() {
                    match self.manager.import_key(&self.key_buffer).await {
                        Ok(imported) => {
                            self.status_message = Some(format!("Imported {} key(s)", imported.len()));
    ...
    async fn handle_export_tab_key(&mut self, key: KeyEvent) -> Result<bool, Box<dyn std::error::Error>> {
        match key.code {
            KeyCode::Enter => {
    ...
    async fn handle_generate_tab_key(&mut self, _key: KeyEvent) -> Result<bool, Box<dyn std::error::Error>> {
    ...
    fn render_tabs(&self, frame: &mut Frame, area: Rect, theme: &Theme) {
        ...
        let current_index = EncryptionTab::all().iter()
            .position(|&t| t == self.current_tab)
            .unwrap_or(0);
        
        let tabs = Tabs::new(tab_names)
            .block(Block::default().borders(Borders::ALL).title("GPG Encryption"))
    ...
    fn render_keys_tab(&mut self, frame: &mut Frame, area: Rect, theme: &Theme) {
        ...
        // Key list
        let key_items: Vec<ListItem> = self.keys.iter().enumerate().map(|(i, key)| {
            ...
            let capabilities = format!(
                "{}{}{}{}",
                if key.capabilities.can_encrypt { "E" } else { "-" },
                if key.capabilities.can_sign { "S" } else { "-" },
                if key.capabilities.can_certify { "C" } else { "-" },
                if key.capabilities.can_authenticate { "A" } else { "-" },
            );
    ...
    fn render_config_tab(&self, frame: &mut Frame, area: Rect, _theme: &Theme) {
        ...
        let config_text: Vec<Line> = config_items.into_iter()
    ...
    fn render_export_tab(&self, frame: &mut Frame, area: Rect, _theme: &Theme) {
        ...
        // Instructions
        let instructions = Paragraph::new("Select a key from the Keys tab and press Enter to export:")
    ...
    fn render_key_details_popup(&self, frame: &mut Frame, area: Rect, theme: &Theme) {
        if let Some(key) = self.keys.get(self.selected_key) {
            ...
            all_lines.push(Line::from(format!("Can Encrypt: {}", key.capabilities.can_encrypt)));
            all_lines.push(Line::from(format!("Can Sign: {}", key.capabilities.can_sign)));
            all_lines.push(Line::from(format!("Can Certify: {}", key.capabilities.can_certify)));
            all_lines.push(Line::from(format!("Can Authenticate: {}", key.capabilities.can_authenticate)));
            
            if let Some(creation) = key.creation_date {
                all_lines.push(Line::from(format!("Created: {}", creation.format("%Y-%m-%d"))));
            }
            
            if let Some(expiration) = key.expiration_date {
                all_lines.push(Line::from(format!("Expires: {}", expiration.format("%Y-%m-%d"))));
            }
