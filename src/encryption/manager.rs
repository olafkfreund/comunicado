...
use super::types::{
    EncryptionResult, EncryptionError, KeyInfo, EncryptionConfig, 
    SecureEmailContent, MessageSecurityStatus
};
use super::gpg::{GpgBackend, SystemGpgBackend, GpgConfig};
...
impl EncryptionManager {
    ...
    pub async fn find_keys_for_emails(&self, emails: &[String]) -> EncryptionResult<Vec<KeyInfo>> {
        ...
        for email in emails {
            if let Some(key) = all_keys.iter().find(|k| {
                k.email.as_ref() == Some(email) || 
                k.user_ids.iter().any(|uid| uid.contains(email))
            }) {
                if key.can_encrypt() {
                    found_keys.push(key.clone());
                } else {
    ...
    pub async fn encrypt_email(
        ...
        if recipient_keys.len() != recipients.len() {
            let missing: Vec<_> = recipients.iter()
                .filter(|email| !recipient_keys.iter().any(|key| key.email.as_ref() == Some(email)))
                .collect();
            return Err(EncryptionError::KeyNotFound(
                format!("Missing keys for: {}", missing.iter().map(|s| s.as_str()).collect::<Vec<_>>().join(", "))
            ));
        }
    ...
    pub async fn decrypt_email(&self, encrypted_content: &str) -> EncryptionResult<SecureEmailContent> {
        debug!("Decrypting email content");
        self.backend.decrypt_email(encrypted_content).await
    }
    
    /// Sign email content
    pub async fn sign_email(&self, content: &str, key_id: Option<&str>) -> EncryptionResult<String> {
        let signer = if let Some(key_id) = key_id {
            key_id.to_string()
        } else {
            // Use default signing key from config
            let config = self.config.read().await;
            config.default_signing_key.clone()
                .ok_or_else(|| EncryptionError::ConfigError("No default signing key configured".to_string()))?
        };
    ...
    pub async fn process_incoming_email(&self, message: &mut StoredMessage) -> EncryptionResult<MessageSecurityStatus> {
        let content = message.body_text.as_ref()
            .ok_or_else(|| EncryptionError::ConfigError("No message content to process".to_string()))?;
        ...
        if should_encrypt {
            ...
            match self.find_keys_for_emails(recipients).await {
                Ok(keys) if keys.len() == recipients.len() => {
                    info!("Encrypting outgoing email for {} recipients", recipients.len());
                    ...
                    match self.encrypt_email(&final_content, recipients, sign_with).await {
                        Ok(encrypted) => {
                            final_content = encrypted;
                            security_status.encryption = super::types::EncryptionStatus::encrypted_and_decrypted(
                                recipients.to_vec(),
    ...
    pub async fn export_key(&self, key_id: &str, include_secret: bool) -> EncryptionResult<String> {
        info!("Exporting key: {} (include_secret: {})", key_id, include_secret);
    ...
    pub async fn generate_key(&self, name: &str, email: &str, comment: Option<&str>) -> EncryptionResult<String> {
