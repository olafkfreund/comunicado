...
use crate::notifications::types::{
    CalendarEventType, EmailEventType, NotificationConfig, NotificationEvent, NotificationPriority,
};
...
impl NotificationIntegrationService {
    ...
    pub async fn handle_new_email(
        ...
        self.email_tracking
            .write()
            .await
            .insert(message.id, tracking_info);
    ...
    pub async fn handle_calendar_event(
        ...
        self.calendar_tracking.write().await.insert(
            event.id.parse().unwrap_or_else(|_| Uuid::new_v4()),
            tracking_info,
        );

        // Schedule reminders
        self.desktop_service
            .schedule_calendar_reminders(event)
            .await?;
        ...
        match event.status {
            ...
            _ => {
                debug!(
                    "Calendar event status doesn't require immediate notification: {:?}",
                    event.status
                );
            }
        }
    ...
    pub async fn handle_calendar_invitation(
        &self,
        event: &Event,
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    pub async fn handle_system_notification(
        &self,
        message: &str,
    ...
    pub async fn notify_sync_complete(
        ...
        let message = if errors > 0 {
            format!(
                "Sync completed for {} with {} new messages and {} errors",
                account_name, new_messages, errors
            )
        } else {
            format!(
                "Sync completed for {} with {} new messages",
                account_name, new_messages
            )
        };
            ...
            loop {
                ...
                for (_, event_tracking) in tracking.iter_mut() {
                    ...
                    for &minutes_before in &event_tracking.reminders_scheduled {
                        let reminder_time = event_tracking.start_time
                            - ChronoDuration::minutes(minutes_before as i64);
                        ...
                        {
                            let event_type = CalendarEventType::EventReminder {
                                minutes_until: minutes_before as i64,
                            };
                            ...
                            if let Err(e) = event_sender.send(notification_event) {
                                ...
                                debug!(
                                    "Sent reminder for event: {} ({} minutes before)",
...
mod tests {
    ...
    fn test_calendar_priority_determination() {
        ...
        assert_eq!(
            service.determine_calendar_priority(&urgent_event),
            NotificationPriority::High
        );
        ...
        assert_eq!(
            service.determine_calendar_priority(&normal_event),
            NotificationPriority::Normal
        );
    }
}
