...
use std::sync::Arc;
use crate::calendar::{Event, CalendarDatabase};
...
impl CalendarNotificationManager {
    ...
    pub async fn notify_event_created(&self, calendar_id: String, event: Event) {
        let notification = CalendarNotification::EventCreated {
            calendar_id,
            event,
        };
    ...
    pub async fn notify_event_updated(&self, calendar_id: String, event: Event) {
        let notification = CalendarNotification::EventUpdated {
            calendar_id,
            event,
        };
    ...
    pub async fn notify_event_reminder(&self, calendar_id: String, event: Event, minutes_until: i64) {
    ...
    pub async fn notify_sync_started(&self, calendar_id: String) {
        let notification = CalendarNotification::SyncStarted {
            calendar_id,
        };
    ...
    pub async fn notify_sync_completed(&self, calendar_id: String, new_count: u32, updated_count: u32) {
    ...
    pub async fn notify_sync_failed(&self, calendar_id: String, error: String) {
        let notification = CalendarNotification::SyncFailed {
            calendar_id,
            error,
        };
    ...
    pub async fn notify_rsvp_sent(&self, event_id: String, response: String) {
        let notification = CalendarNotification::RSVPSent {
            event_id,
            response,
        };
...
mod tests {
    ...
    async fn test_notification_broadcast() {
        ...
        // Send a notification
        manager.notify_event_created("test-calendar".to_string(), event.clone()).await;
        
        // Verify we received the notification
        if let Ok(notification) = receiver.recv().await {
            match notification {
                CalendarNotification::EventCreated { calendar_id, event: received_event } => {
