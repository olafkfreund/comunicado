...
use crate::calendar::{
    CalendarManager, CalendarResult, CalendarError,
    event::Event,
...
impl CalendarSyncEngine {
    ...
    pub async fn update_sync_config(&self, config: CalendarSyncConfig) {
        ...
        if let Some(status) = status_map.get_mut(&config.calendar_id) {
            status.status = if config.enabled { SyncState::Idle } else { SyncState::Disabled };
        }
    ...
    pub async fn sync_calendar(&self, config: CalendarSyncConfig) -> CalendarResult<()> {
        ...
        self.update_sync_status(&calendar_id, SyncState::Syncing, None).await;
        ...
        match result {
            Ok(events_synced) => {
                info!("Successfully synced {} events for calendar: {}", events_synced, calendar_id);
                
                // Update successful sync status
                self.update_sync_status(&calendar_id, SyncState::Completed, None).await;
                ...
                self.update_sync_progress(&calendar_id, final_progress).await;
            }
            Err(e) => {
                error!("Failed to sync calendar {}: {}", calendar_id, e);
                
                // Update error status
                self.update_sync_status(&calendar_id, SyncState::Error, Some(e.to_string())).await;
                ...
                self.update_sync_progress(&calendar_id, error_progress).await;
            }
        }
    ...
    async fn sync_caldav_calendar(
        ...
        self.update_sync_progress(&config.calendar_id, progress).await;
        ...
        for (i, caldav_event) in caldav_events.iter().enumerate() {
            ...
            if i % 10 == 0 {
                let progress = CalendarSyncProgress {
                    ...
                    total_events,
                    started_at: Utc::now(),
                    estimated_completion: Some(Utc::now() + Duration::minutes((total_events - i as u32) as i64 / 10)),
    ...
    async fn update_sync_status(&self, calendar_id: &str, status: SyncState, error: Option<String>) {
        let mut status_map = self.sync_status.write().await;
    ...
    pub async fn force_sync_calendar(&self, calendar_id: &str) -> CalendarResult<()> {
        ...
        if let Some(config) = configs.get(calendar_id) {
            ...
            Err(CalendarError::InvalidData(format!("Calendar {} not found", calendar_id)))
        }
    }
