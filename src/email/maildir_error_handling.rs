...
impl MaildirErrorHandler {
        ...
        match error.kind() {
            ...
            io::ErrorKind::NotFound => {
                ...
                MaildirOperationError::NotFound {
                    message: format!(
                        "File or directory not found during {}: {}",
                        operation, error
                    ),
                    path,
                    suggestion,
    ...
    async fn classify_generic_error(
        ...
        #[cfg(windows)]
        {
            if let Some(raw_error) = error.raw_os_error() {
                match raw_error {
                    5 => {
                        return MaildirOperationError::PermissionDenied {
                    ...
                    32 => {
                        return MaildirOperationError::FileLocked {
                            message: format!(
                                "File is locked or in use during {}: {}",
                                operation, error
                            ),
                            path,
                    ...
                    206 => {
                        return MaildirOperationError::PathTooLong {
                            message: format!("Path too long during {}: {}", operation, error),
                            path: path.clone(),
                            max_length: 260, // Windows MAX_PATH
                            suggestion:
                                "Use shorter file names or move to a directory with a shorter path."
                                    .to_string(),
                        };
                    }
                    _ => {}
                ...
                match raw_error {
                    ...
                    13 => {
                        return MaildirOperationError::PermissionDenied {
                            ...
                            path,
                            suggestion:
                                "Check file permissions. You may need to use 'chmod' or 'sudo'."
                                    .to_string(),
                        }
                    }
                    26 => {
                        return MaildirOperationError::FileLocked {
                            ...
                            path,
                            suggestion:
                                "File is being used by another process. Wait and try again."
                                    .to_string(),
                        }
                    }
                    28 => {
                        return MaildirOperationError::InsufficientSpace {
                            message: format!(
                    ...
                    30 => return MaildirOperationError::ReadOnlyFileSystem {
                        ...
                        path,
                        suggestion:
                            "The file system is mounted read-only. Remount with write permissions."
                                .to_string(),
                    },
                    36 => {
                        return MaildirOperationError::PathTooLong {
        ...
        if self.is_network_path(&path).await {
            return MaildirOperationError::NetworkDriveUnavailable {
                ...
                path,
                suggestion: "Check network connection and ensure the remote drive is accessible."
                    .to_string(),
            };
        }
        ...
        MaildirOperationError::Unknown {
            ...
            source: Some(error),
            suggestion: "This is an unexpected error. Please check the system logs and try again."
                .to_string(),
        }
    }
    ...
    async fn suggest_permission_fix(&self, path: &Path) -> String {
        ...
        if path.is_dir() {
            ...
            if parent.exists() {
                "Cannot create file in this directory. Check directory write permissions."
                    .to_string()
            } else {
    ...
    async fn suggest_not_found_fix(&self, path: &Path, operation: &str) -> String {
        ...
        if operation.contains("import") {
            ...
            "Cannot create export directory. Check the parent directory exists and is writable."
                .to_string()
        } else {
            format!(
                "The path '{}' was not found. Verify the path is correct and accessible.",
                path.display()
            )
        }
    }
    ...
    async fn suggest_already_exists_fix(&self, path: &Path, operation: &str) -> String {
        if operation.contains("export") {
            ...
            format!(
                "'{}' already exists. Use a different name or remove the existing item.",
                path.display()
            )
        }
    }
    ...
    pub fn create_error_report(&self, error: &MaildirOperationError) -> String {
        ...
        match error {
            ...
            MaildirOperationError::Interrupted { progress, .. } => {
                report.push_str(&format!(
                    "• Progress before interruption: {:.1}%\n",
                    progress * 100.0
                ));
            }
            _ => {}
...
mod tests {
    ...
    async fn test_error_classification() {
        ...
        let classified = handler
            .classify_error(perm_error, &test_path, "writing file")
            .await;
    ...
    async fn test_operation_context() {
        let mut context = MaildirOperationContext::new("Import".to_string()).with_paths(
    ...
    async fn test_retry_operation_exhausted() {
        ...
        let result: Result<&str, &str> = handler
            .retry_operation(|| {
                attempt_count += 1;
