...
use crate::imap::{IdleNotification, ImapAccountManager, ImapClient, ImapResult};
...
impl ImapService {
    ...
    pub async fn start_folder_monitoring(
        ...
        // Select the folder
        {
            ...
            client_guard
                .start_folder_monitoring(folder_name.clone())
                .await?;
            ...
            client_guard
                .add_idle_callback(move |notification| {
    ...
    pub async fn sync_folder(
        ...
        info!(
            "Sync would be performed for {}/{} with strategy {:?}",
            account_id, folder_name, strategy
        );
    ...
    async fn get_existing_idle_connection(
        &self,
        account_id: &str,
    ) -> Option<Arc<Mutex<ImapClient>>> {
    ...
    async fn start_idle_processor(&self) {
        ...
        let task = tokio::spawn(async move {
            ...
            while let Some(update) = receiver.recv().await {
                if let Err(e) =
                    Self::process_idle_notification(&update, &sync_engine, &account_manager).await
                {
    ...
    async fn process_idle_notification(
        ...
        match &update.notification {
            ...
            IdleNotification::Expunge { sequence } => {
                info!(
                    "Message expunged from {}/{}: sequence {}",
            ...
            IdleNotification::Fetch { sequence, uid } => {
                info!(
                    "Message flags updated in {}/{}: sequence {} (UID: {:?})",
                        ...
                        if let Some(stats) = stats {
                            if !stats.is_active {
                                ...
                                if let Some(folder_name) = {
                                    ...
                                    let mut client_guard = client.lock().await;
                                    if let Err(e) =
                                        client_guard.start_folder_monitoring(folder_name).await
                                    {
