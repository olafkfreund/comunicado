...
impl EmailDatabase {
    ...
    async fn migrate(&self) -> DatabaseResult<()> {
        ...
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_messages_subject ON messages(subject)")
            .execute(&self.pool)
            .await?;
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_messages_from ON messages(from_addr)")
            .execute(&self.pool)
            .await?;
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_messages_sync ON messages(last_synced)")
            .execute(&self.pool)
            .await?;
        ...
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_drafts_account ON drafts(account_id)")
            .execute(&self.pool)
            .await?;
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_drafts_updated ON drafts(updated_at DESC)")
            .execute(&self.pool)
            .await?;
    ...
    pub async fn store_message(&self, message: &StoredMessage) -> DatabaseResult<()> {
        ...
        match insert_result {
            Ok(_) => {
                tracing::debug!(
                    "Successfully inserted message UID {} for account {} in folder {}",
                    message.imap_uid,
                    message.account_id,
                    message.folder_name
                );
            ...
            {
                tracing::debug!(
                    "Message UID {} already exists, updating instead",
                    message.imap_uid
                );
    ...
    pub async fn get_message_by_uid(
        ...
        folder_name: &str,
        uid: u32,
    ) -> DatabaseResult<Option<StoredMessage>> {
    ...
    pub async fn get_folders(&self, account_id: &str) -> DatabaseResult<Vec<StoredFolder>> {
        ...
        for row in rows {
            ...
            let attributes: Vec<String> =
                serde_json::from_str(&attributes_json).unwrap_or_else(|_| Vec::new());
    ...
    pub async fn get_folder_sync_state(
        ...
        account_id: &str,
        folder_name: &str,
    ) -> DatabaseResult<Option<FolderSyncState>> {
        let row = sqlx::query(
            r"
        ...
        match row {
            Some(row) => {
                ...
                let last_sync: DateTime<Utc> =
                    DateTime::parse_from_rfc3339(row.get("last_sync"))?.into();
                ...
                Ok(Some(FolderSyncState {
                    ...
                    highest_modseq: row
                        .get::<Option<i64>, _>("highest_modseq")
                        .map(|m| m as u64),
                    last_sync,
    ...
    pub async fn delete_messages_by_uids(
        &self,
        account_id: &str,
        folder_name: &str,
        uids: &[u32],
    ...
    pub fn row_to_stored_message(
        &self,
        row: sqlx::sqlite::SqliteRow,
    ) -> DatabaseResult<StoredMessage> {
        ...
        let last_synced: DateTime<Utc> =
            DateTime::parse_from_rfc3339(row.get("last_synced"))?.into();
    ...
    pub async fn get_filter(
        &self,
        filter_id: uuid::Uuid,
    ) -> DatabaseResult<Option<crate::email::EmailFilter>> {
        let row = sqlx::query(
            r"
            SELECT id, name, description, enabled, priority,
                   conditions, actions, created_at, updated_at
    ...
    pub async fn load_draft(&self, draft_id: &str) -> DatabaseResult<Option<StoredDraft>> {
        ...
        if let Some(row) = row {
            ...
            let references: Vec<String> =
                serde_json::from_str(&row.get::<String, _>("draft_references"))?;
    ...
    pub async fn load_drafts_for_account(
        &self,
        account_id: &str,
    ) -> DatabaseResult<Vec<StoredDraft>> {
        let rows = sqlx::query(
            r"
            SELECT id, account_id, subject, to_addrs, cc_addrs, bcc_addrs, reply_to,
                   body_text, body_html, attachments, in_reply_to, draft_references,
    ...
    pub async fn cleanup_old_auto_saved_drafts(
        &self,
    ...
    pub async fn reprocess_message_content(&self) -> DatabaseResult<u32> {
        ...
        for row in rows {
            ...
            if let Some(body_text) = &current_body_text {
                if Self::content_needs_reprocessing(body_text) {
                    ...
                    new_body_text =
                        StoredMessage::parse_and_clean_body_text(&Some(body_text.clone()));
                    needs_update = true;
                }
            }
            ...
            if let Some(body_html) = &current_body_html {
                if Self::content_needs_reprocessing(body_html) {
                    ...
                    new_body_html =
                        StoredMessage::parse_and_clean_body_html(&Some(body_html.clone()));
                    needs_update = true;
                }
            }
        ...
        tracing::info!(
            "Completed message content reprocessing. {} messages updated",
            processed_count
        );
        Ok(processed_count)
    }
...
impl StoredMessage {
        ...
        Self {
            ...
            to_addrs: envelope
                .map(|env| {
                    env.to
                        .iter()
                        .filter_map(|addr| addr.email_address())
                        .collect()
                })
                .unwrap_or_default(),
            cc_addrs: envelope
    ...
    fn parse_and_clean_body_text(raw_body: &Option<String>) -> Option<String> {
        ...
        tracing::debug!(
            "Parsing body text, raw content length: {}",
            raw_content.len()
        );
        ...
        if has_html_tags || crate::html::is_html_content(&cleaned_content) {
            tracing::info!(
                "Converting HTML content to plain text for body_text (length: {})",
                cleaned_content.len()
            );
            ...
            if !plain_text.trim().is_empty() && plain_text.len() > 10 {
                tracing::info!(
                    "HTML-to-text conversion successful: {} chars -> {} chars",
                    cleaned_content.len(),
                    plain_text.len()
                );
                Some(plain_text)
    ...
    fn parse_and_clean_body_html(raw_body: &Option<String>) -> Option<String> {
        ...
        tracing::debug!(
            "Parsing HTML body, raw content length: {}",
            raw_content.len()
        );
    ...
    fn clean_raw_email_content(raw_content: &str) -> String {
        tracing::debug!(
            "Cleaning raw email content of length: {}",
            raw_content.len()
        );
    ...
    fn extract_text_from_html(html_content: &str) -> String {
        ...
        tracing::debug!(
            "Manual HTML text extraction complete: {} chars",
            result.len()
        );
        result
    }
    ...
    fn aggressive_content_extraction(raw_content: &str) -> String {
        ...
        for (i, line) in lines.iter().enumerate() {
            ...
            let is_header = header_patterns.iter().any(|&pattern| {
                line_lower.starts_with(pattern) || line_lower.contains(&format!(" {}", pattern))
                // Headers can be indented
            });
            ...
            if line.starts_with("--")
                && (
                    line.contains("Apple-Mail")
                        ...
                        || line.contains("=_")
                        || line.len() > 15
                    // Long boundary markers
                )
            {
                tracing::debug!(
                    "Skipping boundary line {}: {}",
                    i,
                    &line[..std::cmp::min(40, line.len())]
                );
                continue;
            }
            ...
            if line.len() > 100 && !line.contains(" ") && !line.starts_with("http") {
                let non_alnum_count = line
                    .chars()
                    .filter(|c| !c.is_alphanumeric() && *c != '/' && *c != '+' && *c != '=')
                    .count();
                if non_alnum_count < 5 {
                    // Looks like encoded data
                    tracing::debug!(
                        "Skipping encoded data line {}: {}",
                        i,
                        &line[..std::cmp::min(40, line.len())]
                    );
                    continue;
            ...
            if line.len() > 50 {
                let technical_chars = line
                    .chars()
                    .filter(|&c| c == '=' || c == ';' || c == ':')
                    .count();
                if technical_chars > line.len() / 10 {
                    // More than 10% technical characters
                    tracing::debug!(
                        "Skipping technical data line {}: {}",
                        i,
                        &line[..std::cmp::min(40, line.len())]
                    );
                    continue;
        ...
        tracing::debug!(
            "Aggressive extraction result: {} lines, {} chars (from {} input chars)",
            result_lines.len(),
            result.len(),
            raw_content.len()
        );
...
mod tests {
    ...
    async fn test_message_storage_and_retrieval() {
        ...
        // Retrieve message
        let retrieved = db
            .get_message_by_uid("test-account", "INBOX", 1)
            .await
            .unwrap();
        ...
        let messages = db
            .get_messages("test-account", "INBOX", None, None)
            .await
            .unwrap();
    ...
    async fn test_full_text_search() {
        ...
        // Search by subject
        let results = db
            .search_messages("test-account", "meeting", None)
            .await
            .unwrap();
        ...
        // Search by body
        let results = db
            .search_messages("test-account", "quarterly", None)
            .await
            .unwrap();
        assert_eq!(results.len(), 1);
        ...
        // Search by sender
        let results = db
            .search_messages("test-account", "boss", None)
            .await
            .unwrap();
        assert_eq!(results.len(), 1);
    }
