...
impl AttachmentViewer {
    ...
    pub async fn view_attachment(&mut self, attachment: &AttachmentInfo, data: &[u8]) -> ViewResult {
    ...
    async fn view_image_attachment(&mut self, attachment: &AttachmentInfo, data: &[u8]) -> ViewResult {
        match self.image_manager.load_image_from_bytes(data, Some(&attachment.stored.content_type)).await {
            Ok(rendered_content) => {
            ...
            Err(e) => {
                let error_lines = vec![
                    Line::styled("🖼️ Image Viewer", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)),
                    Line::raw(""),
                    Line::styled(format!("Failed to load image: {}", e), Style::default().fg(Color::Red)),
                    Line::raw(""),
                    Line::styled("File Information:", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
                    Line::raw(format!("  Filename: {}", attachment.display_name)),
                    Line::raw(format!("  Type: {}", attachment.attachment_type.description())),
    ...
    fn view_text_attachment(&mut self, attachment: &AttachmentInfo, data: &[u8]) -> ViewResult {
        ...
        let mut lines = vec![
            Line::styled("📄 Text Viewer", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)),
            Line::raw(format!("File: {} ({})", attachment.display_name, attachment.format_size())),
            Line::raw(""),
        ];
        ...
        if !language.is_empty() {
            lines.push(Line::styled(format!("Language: {}", language), Style::default().fg(Color::Green)));
            lines.push(Line::raw(""));
        }
        
        // Add content lines with basic syntax coloring
        for (line_num, content_line) in text_content.lines().enumerate().take(1000) { // Limit lines for performance
        ...
        if text_content.lines().count() > 1000 {
            lines.push(Line::raw(""));
            lines.push(Line::styled("... (content truncated)", Style::default().fg(Color::Yellow)));
        }
    ...
    fn view_preview_attachment(&mut self, attachment: &AttachmentInfo, data: &[u8]) -> ViewResult {
        let lines = vec![
            Line::styled("📋 File Preview", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)),
            Line::raw(""),
            Line::styled("File Information", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
            ...
            Line::raw(format!("  🔒 Safe: {}", if attachment.is_safe { "Yes" } else { "⚠️  No" })),
            Line::raw(""),
            Line::styled("Content Summary", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
        ];
        ...
        match attachment.attachment_type {
            ...
            AttachmentType::Zip | AttachmentType::Rar | AttachmentType::SevenZip | AttachmentType::Tar => {
        ...
        content_lines.push(Line::styled("Actions", Style::default().fg(Color::Green).add_modifier(Modifier::BOLD)));
    ...
    fn view_info_attachment(&mut self, attachment: &AttachmentInfo) -> ViewResult {
        let lines = vec![
            Line::styled("ℹ️  File Information", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)),
            Line::raw(""),
            Line::raw(format!("  📄 Filename: {}", attachment.display_name)),
            Line::raw(format!("  📊 Type: {}", attachment.attachment_type.description())),
            Line::raw(format!("  📏 Size: {}", attachment.format_size())),
            Line::raw(format!("  🏷️  MIME Type: {}", attachment.stored.content_type)),
            Line::raw(format!("  🔒 Safety: {}", if attachment.is_safe { "Safe" } else { "⚠️  Potentially unsafe" })),
            Line::raw(""),
            Line::styled("This file type cannot be viewed in the terminal.", Style::default().fg(Color::Yellow)),
            Line::raw("Use 's' to save the file and open it with an external application."),
            Line::raw(""),
            Line::styled("Actions", Style::default().fg(Color::Green).add_modifier(Modifier::BOLD)),
    ...
    fn should_show_line_numbers(&self, language: &str) -> bool {
        matches!(language, "rust" | "python" | "javascript" | "c" | "cpp" | "go" | "java" | "json" | "xml")
    }
    ...
    fn preview_pdf_info(&self, data: &[u8]) -> Vec<Line<'static>> {
        ...
        if data.len() >= 4 && &data[0..4] == b"%PDF" {
            ...
            lines.push(Line::styled("  ⚠️  Invalid PDF format", Style::default().fg(Color::Red)));
        }
    ...
    fn preview_office_info(&self, attachment_type: &AttachmentType, data: &[u8]) -> Vec<Line<'static>> {
        let mut lines = vec![];
        ...
        if is_office_format {
            ...
            lines.push(Line::raw("  💡 Use Microsoft Office or LibreOffice to open"));
        } else {
            lines.push(Line::styled("  ⚠️  Unrecognized Office format", Style::default().fg(Color::Yellow)));
        }
    ...
    fn preview_archive_info(&self, attachment_type: &AttachmentType, data: &[u8]) -> Vec<Line<'static>> {
        let mut lines = vec![];
        ...
        if is_valid {
            ...
            lines.push(Line::raw("  💡 Extract with archive utility to view contents"));
            lines.push(Line::raw("  ⚠️  Always scan archives before extracting"));
        } else {
            lines.push(Line::styled(format!("  ⚠️  Invalid {} format", archive_type), Style::default().fg(Color::Red)));
        }
    ...
    pub fn render(&mut self, frame: &mut Frame, area: Rect, block: Block, theme: &Theme) {
        ...
        let lines = if let Some(ref content) = self.current_content {
            ...
            vec![
                Line::styled("No attachment loaded", Style::default().fg(theme.colors.palette.text_muted))
            ]
        };
        ...
        let scroll_indicator = if let Some(ref content) = self.current_content {
            if content.len() > content_height {
                let position = (self.scroll as f32 / (content.len() - content_height) as f32 * 100.0) as u16;
                format!(" ({}%)", position)
            } else {
                String::new()
            }
    ...
    pub async fn create_temp_file(&self, attachment: &AttachmentInfo, data: &[u8]) -> Result<std::path::PathBuf, Box<dyn std::error::Error>> {
