use crate::email::{EmailDatabase, ImportConfig, ImportStats, MaildirImportError, MaildirImporter};
use anyhow::Result;
...
impl ImportWizard {
            ...
            KeyCode::Char(' ') => {
                ...
                if let Some(selected) = self.state.folder_list_state.selected() {
                    if selected < self.state.folder_selection_states.len() {
                        self.state.folder_selection_states[selected] =
                            !self.state.folder_selection_states[selected];
                    }
                }
            }
    ...
    async fn handle_configuration_keys(&mut self, key: KeyEvent) -> ImportWizardResult<()> {
        match key.code {
            ...
            KeyCode::Char('d') => {
                ...
                self.state.import_config.skip_duplicates =
                    !self.state.import_config.skip_duplicates;
            }
            KeyCode::Char('v') => {
                // Toggle validation
                self.state.import_config.validate_format =
                    !self.state.import_config.validate_format;
            }
            KeyCode::Char('t') => {
                // Toggle preserve timestamps
                self.state.import_config.preserve_timestamps =
                    !self.state.import_config.preserve_timestamps;
            }
            _ => {}
        }
    ...
    async fn refresh_directory_list(&mut self) -> ImportWizardResult<()> {
        ...
        dir_entries.sort_by(|a, b| match (a.is_maildir, b.is_maildir) {
        ...
        // Update scroll state
        self.state.directory_scroll_state =
            ScrollbarState::default().content_length(self.state.directories.len());
    ...
    async fn scan_maildir_folders(&mut self) -> ImportWizardResult<()> {
        ...
        // Update scroll state
        self.state.folder_scroll_state =
            ScrollbarState::default().content_length(self.state.maildir_folders.len());
    ...
    async fn start_import(&mut self) -> ImportWizardResult<()> {
        if let Some(ref directory) = self.state.selected_directory {
            let selected_folders: Vec<_> = self
                .state
                .maildir_folders
                .iter()
            ...
            if selected_folders.is_empty() {
                return Err(ImportWizardError::Ui(
                    "No folders selected for import".to_string(),
                ));
            }
            ...
            let handle = tokio::spawn(async move {
                ...
                importer
                    .import_from_directory(&directory, &account_id)
                    .await
    ...
    fn render_directory_selection(&mut self, f: &mut Frame, area: Rect) {
        ...
        let info_text = format!(
            "Current Directory: {}",
            self.state.current_directory.display()
        );
        ...
        if self.state.directories.len() > main_chunks[1].height as usize {
            ...
            f.render_stateful_widget(
                scrollbar,
                scrollbar_area,
                &mut self.state.directory_scroll_state,
            );
        }
    }
    ...
    fn render_configuration(&mut self, f: &mut Frame, area: Rect) {
        ...
        let config_text = vec![
            Line::from(vec![
                ...
                Span::styled(
                    if self.state.import_config.skip_duplicates {
                        "Yes"
                    } else {
                        "No"
                    },
                    Style::default().fg(if self.state.import_config.skip_duplicates {
                        Color::Green
                    } else {
                        Color::Red
                    }),
            ...
            Line::from(vec![
                ...
                Span::styled(
                    if self.state.import_config.validate_format {
                        "Yes"
                    } else {
                        "No"
                    },
                    Style::default().fg(if self.state.import_config.validate_format {
                        Color::Green
                    } else {
                        Color::Red
                    }),
            ...
            Line::from(vec![
                ...
                Span::styled(
                    if self.state.import_config.preserve_timestamps {
                        "Yes"
                    } else {
                        "No"
                    },
                    Style::default().fg(if self.state.import_config.preserve_timestamps {
                        Color::Green
                    } else {
                        Color::Red
                    }),
        ...
        let total_messages: usize = selected_folders
            .iter()
            .map(|(folder, _)| folder.message_count)
            .sum();
        ...
        let summary_lines = vec![
            Line::from(format!(
                "Directory: {}",
                self.state.selected_directory.as_ref().unwrap().display()
            )),
    ...
    fn render_import_progress(&mut self, f: &mut Frame, area: Rect) {
        if let Some(ref progress) = self.state.import_progress {
            ...
            let details = vec![
                ...
                Line::from(format!(
                    "Folders: {}/{}",
                    progress.folders_processed, progress.total_folders
                )),
    ...
    fn render_completion(&mut self, f: &mut Frame, area: Rect) {
        if let Some(ref stats) = self.state.import_stats {
            ...
            let success_text = vec![
                ...
                Line::from(format!(
                    "Maildir Folders Found: {}",
                    stats.maildir_folders_found
                )),
...
mod tests {
    ...
    async fn test_wizard_state_transitions() {
        ...
        wizard.state.maildir_folders = vec![MaildirFolderEntry {
