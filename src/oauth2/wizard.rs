use crate::oauth2::{
    AccountConfig, OAuth2Client, OAuth2Error, OAuth2Provider, OAuth2Result, ProviderConfig,
    ProviderDetector, SecureStorage,
};
...
impl SetupWizard {
    ...
    ) -> OAuth2Result<Option<AccountConfig>> {
            ...
            match self.state {
                ...
                WizardState::Testing => {
                    ...
                    if let Err(e) = self.test_configuration().await {
                        self.state =
                            WizardState::Error(format!("Configuration test failed: {}", e));
                        continue;
                    }
                }
    ...
    async fn handle_key_event(&mut self, key: KeyEvent) -> OAuth2Result<bool> {
        ...
        match (&self.state, &self.input_mode) {
            (WizardState::Welcome, _) => match key.code {
            ...
            (WizardState::EmailInput, InputMode::Email) => match key.code {
                KeyCode::Enter => {
                    if !self.email_input.is_empty() && self.email_input.contains('@') {
                        self.selected_provider =
                            ProviderDetector::detect_from_email(&self.email_input);
            ...
            (WizardState::EmailInput, InputMode::DisplayName) => match key.code {
                KeyCode::Enter => {
                    if !self.email_input.is_empty() && self.email_input.contains('@') {
                        self.selected_provider =
                            ProviderDetector::detect_from_email(&self.email_input);
            ...
            (WizardState::ProviderSelection, _) => match key.code {
                KeyCode::Up => {
            ...
            (WizardState::ProviderInstructions, _) => match key.code {
                KeyCode::Enter | KeyCode::Char(' ') => {
            ...
            (WizardState::CredentialsInput, InputMode::ClientId) => match key.code {
            ...
            (WizardState::CredentialsInput, InputMode::ClientSecret) => match key.code {
                KeyCode::Enter => {
                    if !self.client_id_input.is_empty() {
            ...
            (WizardState::Error(_), _) => match key.code {
                KeyCode::Enter | KeyCode::Char(' ') => {
                    self.state = WizardState::Welcome;
                    self.reset_inputs();
                }
    ...
    async fn setup_oauth_client(&mut self) -> OAuth2Result<()> {
        if let Some(provider) = &self.selected_provider {
            let config = ProviderConfig::get_config(provider)?.with_credentials(
    ...
    async fn handle_authorization(&mut self) -> OAuth2Result<()> {
        if let Some(oauth_client) = &mut self.oauth_client {
            ...
            self.account_config = Some(
                oauth_client
                    .create_account_config(&token_response, display_name)
                    .await?,
            );
    ...
    fn draw_provider_selection(&mut self, f: &mut Frame, area: Rect) {
        ...
        let items: Vec<ListItem> = providers
            .iter()
            .map(|provider| ListItem::new(provider.display_name()))
            .collect();
