use crate::oauth2::{
    AccountConfig, AuthorizationCode, OAuth2Error, OAuth2Result, PkceChallenge, ProviderConfig,
    TokenResponse,
};
...
impl OAuth2Client {
    ...
    pub async fn start_authorization(&mut self) -> OAuth2Result<AuthorizationRequest> {
        ...
        let actual_redirect_uri = if self.config.redirect_uri.starts_with("http://localhost")
            && !self.config.redirect_uri.contains(":")
        {
    ...
    pub async fn exchange_code(
        ...
        let token_data: Value = response.json().await.map_err(OAuth2Error::NetworkError)?;
    ...
    pub async fn get_user_info(&self, access_token: &str) -> OAuth2Result<UserInfo> {
        ...
        let user_data: Value = response.json().await.map_err(OAuth2Error::NetworkError)?;
        ...
        let (email, name) = match self.config.provider {
            crate::oauth2::OAuth2Provider::Gmail => {
                let email = data["email"]
                    .as_str()
                    .ok_or_else(|| {
                        OAuth2Error::InvalidToken("Missing email in user info".to_string())
                    })?
                    .to_string();
            ...
            crate::oauth2::OAuth2Provider::Outlook => {
                let email = data["mail"]
                    ...
                    .to_string();
                let name = data["displayName"].as_str().unwrap_or(&email).to_string();
                (email, name)
            }
            crate::oauth2::OAuth2Provider::Yahoo => {
                let email = data["email"]
...
impl CallbackServer {
    ...
    fn new_with_dynamic_port() -> OAuth2Result<Self> {
        ...
        let listener = TcpListener::bind("127.0.0.1:0").map_err(|e| {
            OAuth2Error::StorageError(format!("Failed to bind to any available port: {}", e))
        })?;

        let port = listener
            .local_addr()
            .map_err(|e| OAuth2Error::StorageError(format!("Failed to get local address: {}", e)))?
            .port();
        ...
        if let Some(receiver) = &mut self.receiver {
            match receiver.try_recv() {
                ...
                Err(tokio::sync::oneshot::error::TryRecvError::Closed) => Some(Err(
                    OAuth2Error::AuthorizationFailed("Callback server closed".to_string()),
                )),
            }
        } else {
            None
    ...
    async fn wait_for_callback(&mut self) -> OAuth2Result<AuthorizationCode> {
        if let Some(receiver) = self.receiver.take() {
            ...
            Err(OAuth2Error::AuthorizationFailed(
                "No callback receiver available".to_string(),
            ))
        }
    }
    ...
    fn parse_callback_from_request_static(request: &str) -> OAuth2Result<AuthorizationCode> {
        ...
        let url_part = first_line.split_whitespace().nth(1).ok_or_else(|| {
            OAuth2Error::AuthorizationFailed("Invalid HTTP request format".to_string())
        })?;

        // Parse URL
        let full_url = format!("http://localhost{}", url_part);
        let url = Url::parse(&full_url).map_err(|e| {
            OAuth2Error::AuthorizationFailed(format!("Failed to parse callback URL: {}", e))
        })?;
