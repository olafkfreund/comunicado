    ...
    pub fn setup_instructions(&self) -> Vec<String> {
        match &self.provider {
            OAuth2Provider::Gmail => vec![
                ...
                "10. Under 'Authorized redirect URIs', click 'ADD URI'".to_string(),
                "11. Add these redirect URIs (one at a time):".to_string(),
                ...
                "    ... (up to 8089 if you want full coverage)".to_string(),
                "12. Click 'CREATE'".to_string(),
                "13. Copy the 'Client ID' from the popup (Client Secret optional)".to_string(),
                "".to_string(),
                "Note: Gmail supports PKCE, so Client Secret is optional for security.".to_string(),
            ],
