...
impl ProviderConfig {
    ...
    pub fn gmail() -> Self {
        Self {
            ...
            redirect_uri: "http://localhost".to_string(), // Desktop apps use flexible localhost
            scopes: vec![
    ...
    pub fn setup_instructions(&self) -> Vec<String> {
        match &self.provider {
            OAuth2Provider::Gmail => vec![
                ...
                "3. In the sidebar, go to 'APIs & Services' > 'Library'".to_string(),
                "4. Search for 'Gmail API' and click 'Enable'".to_string(),
                "5. Go to 'APIs & Services' > 'Credentials'".to_string(),
                "6. Click '+ CREATE CREDENTIALS' > 'OAuth client ID'".to_string(),
                "7. If prompted, configure the OAuth consent screen first".to_string(),
                "8. Select 'Desktop application' as the application type".to_string(),
                "9. Give your OAuth client a name (e.g., 'Comunicado')".to_string(),
                "10. Click 'CREATE' (no redirect URI needed for desktop apps)".to_string(),
                "11. Copy the 'Client ID' from the popup".to_string(),
                "12. Optionally copy the 'Client Secret' (recommended for security)".to_string(),
                "".to_string(),
                "Important: Desktop applications automatically use localhost redirects.".to_string(),
                "Google will handle the redirect URI automatically for your app.".to_string(),
                "".to_string(),
                "Troubleshooting:".to_string(),
                "• If you see 'redirect_uri_mismatch' error, ensure you selected".to_string(),
                "  'Desktop application' (not 'Web application') in step 8".to_string(),
                "• Desktop apps don't need manual redirect URI configuration".to_string(),
                "• Google automatically allows localhost redirects for desktop apps".to_string(),
                "".to_string(),
                "Note: Gmail supports PKCE, making Client Secret optional but recommended.".to_string(),
            ],
