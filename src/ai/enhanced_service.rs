...
use crate::ai::{
    AIBackgroundProcessor, AIError, AIOperation, AIOperationType, AIResult, AIService,
    AIStreamingManager, BackgroundConfig, OperationPriority, OperationResult, ProgressUpdate,
    StreamChunk, StreamingConfig,
};
...
impl EnhancedAIService {
    ...
    pub async fn new(ai_service: Arc<AIService>, config: EnhancedAIConfig) -> AIResult<Self> {
        ...
        let streaming_manager = Arc::new(AIStreamingManager::new(config.streaming_config.clone()));
    ...
    pub async fn process_request(
        ...
        debug!(
            "Processing AI request {} with priority {:?}",
            operation_id, request.priority
        );
        ...
        let result = if request.enable_background {
            ...
            self.process_background_request(operation, request.enable_streaming)
                .await
        } else {
            // Process immediately
    ...
    async fn process_immediate_request(&self, request: &EnhancedAIRequest) -> AIResult<String> {
        match &request.operation_type {
            ...
            AIOperationType::BatchEmailProcessing { .. } => Err(AIError::internal_error(
                "Batch processing requires background mode",
            )),
        }
    }
        ...
        tokio::spawn(async move {
            ...
            if let Some(mut receiver) = rx {
                while let Some(progress) = receiver.recv().await {
                    debug!(
                        "Progress update for operation {}: {:.1}% - {}",
    ...
    async fn update_metrics(&self, _processing_time: Duration, success: bool) {
        ...
        if !success {
            // Update error rate
            let error_count =
                (metrics.error_rate * (metrics.total_operations - 1) as f64 / 100.0) + 1.0;
    ...
    pub async fn get_operation_status(
        &self,
        operation_id: Uuid,
    ) -> Option<crate::ai::OperationStatus> {
        self.background_processor
...
mod tests {
    use super::*;
    use crate::ai::{AIConfig, AIProviderManager, AIProviderType, AIResponseCache, AIService};
    ...
    async fn create_test_enhanced_service() -> EnhancedAIService {
        ...
        let provider_manager = Arc::new(tokio::sync::RwLock::new(AIProviderManager::new(
        ...
        EnhancedAIService::new(ai_service, enhanced_config)
            .await
            .unwrap()
    }
