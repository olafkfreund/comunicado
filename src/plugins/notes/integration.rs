...
use super::watcher::{FileWatcher, WatcherConfig, FileSystemEvent, EventBatch};
...
impl FileSystemMonitor {
    ...
    pub fn with_configs(watcher_config: WatcherConfig, scan_config: ScanConfig) -> NoteResult<Self> {
        let watcher = FileWatcher::with_config(watcher_config)?;
        let scanner = DirectoryScanner::with_config(scan_config);
    ...
    pub fn add_directory(&mut self, directory: WatchedDirectory) -> NoteResult<()> {
        ...
        println!("Initial scan found {} note files, {} new files", 
    ...
    pub async fn start_monitoring(&mut self) -> NoteResult<tokio::sync::mpsc::UnboundedReceiver<EventBatch>> {
...
impl ProcessingResult {
    ...
    pub fn file_changes(&self) -> usize {
        self.created_files.len() + self.modified_files.len() + self.deleted_files.len() + self.moved_files.len()
    }
}
...
mod tests {
    ...
    async fn test_file_monitoring_and_event_processing() {
        ...
        if let Ok(Some(batch)) = batch_result {
            ...
            assert!(processing_result.created_files.iter().any(|p| p == &new_file));
        } else {
    ...
    async fn test_batch_processing_multiple_files() {
        ...
        if let Ok(Some(batch)) = batch_result {
            ...
            println!("Batch processed {} events, {} file changes", 
                    processing_result.batch_size, processing_result.file_changes());
        } else {
    ...
    async fn test_debouncing_performance() {
        ...
        for i in 0..5 {
            create_test_file(temp_dir.path(), &format!("rapid{}.md", i), &format!("# Rapid {}", i)).await;
        }
        ...
        loop {
            ...
            if let Ok(Some(batch)) = batch_result {
                ...
                println!("Batch {}: {} file changes, processing time: {}ms", 
                        batch_count, processing_result.file_changes(), processing_result.processing_time_ms);
            } else {
        ...
        println!("Total: {} events in {} batches over {:?}", 
