...
mod tests {
    ...
    fn create_test_files_and_dirs(temp_dir: &Path) -> Vec<PathBuf> {
        ...
        let hidden_files = vec![".hidden_note.md", ".secret.txt", ".config.md"];
    ...
    fn test_directory_scanner_ignore_patterns() {
        ...
        let watched_dir =
            WatchedDirectory::new(temp_dir.path().to_path_buf(), "Ignore Test".to_string());
        ...
        for expected in expected_files {
            assert!(
                found_paths.contains(&expected.to_string()),
                "Expected file {} not found. Found: {:?}",
                expected,
                found_paths
            );
        }
        ...
        for ignored in ignored_files {
            assert!(
                !found_paths.contains(&ignored.to_string()),
                "Ignored file {} was found when it shouldn't be",
                ignored
            );
        }
    }
    ...
    fn test_scan_config_with_hidden_files() {
        ...
        let watched_dir =
            WatchedDirectory::new(temp_dir.path().to_path_buf(), "Hidden Test".to_string());
    ...
    fn test_file_size_filtering() {
        ...
        let watched_dir =
            WatchedDirectory::new(temp_dir.path().to_path_buf(), "Size Test".to_string());
    ...
    fn test_complex_ignore_patterns() {
        ...
        for (path, should_ignore) in test_cases {
            ...
            assert_eq!(
                ...
                "Path '{}' should_ignore={}, but got {}",
                path, should_ignore, result
            );
        }
    }
    ...
    fn test_performance_with_many_patterns() {
        ...
        let test_paths = (0..1000)
            .map(|i| format!("/path/to/file{}.md", i))
            .collect::<Vec<_>>();
