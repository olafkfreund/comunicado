    ...
    async fn create_test_notes_in_dir(temp_dir: &Path) -> NoteResult<Vec<PathBuf>> {
        ...
        for note in test_notes {
            ...
            let mut file = File::create(&file_path).map_err(|e| {
                NoteError::FileSystem(format!("Failed to create {}: {}", file_path.display(), e))
            })?;

            file.write_all(note.content.as_bytes()).map_err(|e| {
                NoteError::FileSystem(format!("Failed to write {}: {}", file_path.display(), e))
            })?;
    ...
    fn create_search_test_cases() -> Vec<SearchTestCase> {
        vec![
            ...
            SearchTestCase {
                ...
                expected_count: 2,
                expected_titles: vec![
                    "Quick Note on Performance".to_string(),
                    "Memory Management in Rust".to_string(),
                ],
...
"#;
        ...
        assert_eq!(
            parsed.frontmatter.as_ref().unwrap().title,
            Some("Complex Note".to_string())
        );
