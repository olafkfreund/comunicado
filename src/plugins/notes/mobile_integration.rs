...
use crate::mobile::{
    kde_connect::types::{ContactInfo, SmsMessage},
...
impl MobileNotesIntegration {
    ...
    async fn evaluate_sms_for_conversion(
        &self,
        message: &SmsMessage,
    ) -> NoteResult<Option<SmsConversionCandidate>> {
        ...
        if self
            .is_sms_already_converted(&message.id.to_string())
            .await?
        {
            return Ok(None);
        }
    ...
    pub async fn convert_sms_to_note(
        &self,
        candidate: &SmsConversionCandidate,
    ) -> NoteResult<Note> {
        ...
        let content = self
            ...
            .replace(
                "{{date}}",
                &message_date.format("%Y-%m-%d %H:%M:%S").to_string(),
            )
        ...
        frontmatter.set_metadata(
            "sms_id".to_string(),
            serde_yaml::Value::String(candidate.message.id.to_string()),
        );
        frontmatter.set_metadata(
            "sender".to_string(),
            serde_yaml::Value::String(sender_phone.clone()),
        );
        frontmatter.set_metadata(
        ...
        // Create the note
        let file_path = self.config.mobile_notes_directory.join(format!(
            "{}.md",
            candidate.suggested_title.replace(' ', "-").to_lowercase()
        ));
        ...
        if let Some(contact) = &candidate.contact {
            ...
            self.link_note_to_contact(&note_id, display_name, sender_phone)
                .await?;
        }
    ...
    pub async fn link_note_to_contact(
        &self,
        note_id: &NoteId,
        contact_name: &str,
        phone: &str,
    ...
    pub async fn send_mobile_notification(
        &self,
        note_id: &NoteId,
        ...
        info!(
            "Mobile notification requested for note {}: {}",
            note_id, message
        );
        ...
        let _ = self
            .notification_tx
            .send(MobileNoteEvent::NotificationSent {
                note_id: note_id.clone(),
    ...
    pub async fn share_note_to_mobile(&self, note_id: &NoteId, device_id: &str) -> NoteResult<()> {
        ...
        tokio::fs::write(&temp_file, &note.content)
            .await
            .map_err(|e| super::manager::NoteError::Io(e))?;

        // For now, just log the file sharing request
        info!(
            "File sharing requested for note {} to device {}: {}",
            note_id, device_id, temp_file
        );
    ...
    async fn is_sms_already_converted(&self, sms_id: &str) -> NoteResult<bool> {
        ...
        let results = self
            .note_storage
            .search_notes(&format!("sms_id:{}", sms_id), 1)
            .await?;
        Ok(!results.is_empty())
    }
    ...
    fn generate_note_title_from_sms(
        &self,
        message: &SmsMessage,
