...
impl ScannedFile {
    ...
    pub fn new(path: PathBuf, metadata: &Metadata) -> NoteResult<Self> {
        let modified = metadata.modified()
            .map_err(|e| NoteError::FileSystem(format!("Failed to get modified time for {}: {}", path.display(), e)))?;
    ...
    fn calculate_content_hash(path: &Path) -> NoteResult<String> {
        let mut file = File::open(path)
            .map_err(|e| NoteError::FileSystem(format!("Failed to open file {}: {}", path.display(), e)))?;
        ...
        loop {
            let bytes_read = file.read(&mut buffer)
                .map_err(|e| NoteError::FileSystem(format!("Failed to read file {}: {}", path.display(), e)))?;
...
impl DirectoryScanner {
            ...
            match entry {
                Ok(entry) => {
                    if let Err(e) = self.process_entry(&entry, &mut result, &mut current_paths) {
                        result.add_error(format!("Error processing {}: {}", entry.path().display(), e));
                    }
                }
                Err(e) => {
    ...
    fn process_entry(
        ...
        let metadata = entry.metadata()
            .map_err(|e| NoteError::FileSystem(format!("Failed to get metadata for {}: {}", path.display(), e)))?;
    ...
    pub fn last_scan_stats(&self) -> (usize, usize) {
        ...
        let note_files = self.previous_scan.values()
...
mod tests {
    ...
    fn test_scanned_file_creation() {
        ...
        let file_path = create_test_file(temp_dir.path(), "test.md", "# Test Note\n\nContent here.");
    ...
    fn test_directory_scanner_creation() {
        ...
        assert_eq!(custom_scanner.config.max_file_size, custom_config.max_file_size);
    }
