...
use super::tui::{NoteTUI, PopupState, TUIMode};
...
impl NoteTUI {
    ...
    fn render_settings_mode(&self, f: &mut Frame, area: Rect) {
        let settings_text = format!(
            ...
            if self.config().vim_keybindings {
                "Yes"
            } else {
                "No"
            },
    ...
    fn render_input_dialog(
        ...
        let prompt_paragraph =
            Paragraph::new(prompt).style(Style::default().fg(self.config().theme.text));
    ...
    fn render_error_dialog(&self, f: &mut Frame, message: &str) {
        ...
        let block = Block::default().title("Error").borders(Borders::ALL).style(
            Style::default()
                .bg(self.config().theme.background)
                .fg(self.config().theme.error),
        );
    ...
    fn get_keybindings_for_mode(&self) -> Vec<String> {
        match self.current_mode() {
            ...
            TUIMode::Edit => vec!["Ctrl+S:Save".to_string(), "Esc:View".to_string()],
            TUIMode::Search => vec!["Enter:Search".to_string(), "Esc:Browse".to_string()],
            TUIMode::Create => vec!["Ctrl+S:Save".to_string(), "Esc:Cancel".to_string()],
            TUIMode::Settings => vec!["b:Back".to_string(), "Esc:Browse".to_string()],
            TUIMode::Help => vec!["Any key:Back".to_string()],
        }
    }
}
