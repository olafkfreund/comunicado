...
impl NoteTUI {
    ...
    async fn handle_popup_keys(&mut self, key: KeyEvent) -> NoteResult<bool> {
        if let Some(popup) = self.popup.clone() {
            match popup {
                PopupState::ConfirmDelete { note_id } => match key.code {
                ...
                PopupState::CreateNote { mut title_input } => match key.code {
                    KeyCode::Enter => {
    ...
    async fn refresh_notes(&mut self) -> NoteResult<()> {
        self.notes = self
            .storage
            .get_recent_notes(self.config.max_notes_display)
            .await?;
        self.list_state
            .select(if self.notes.is_empty() { None } else { Some(0) });
...
mod tests {
    ...
    async fn test_mode_switching() {
        ...
        // Test help mode
        tui.handle_key(KeyEvent::new(KeyCode::F(1), KeyModifiers::NONE))
            .await
            .unwrap();
