...
impl EventBus {
    ...
    fn process_event(&mut self, mut boxed_event: BoxedEvent) -> Result<(), EventError> {
        ...
        // Apply middleware (before)
        {
            ...
            for (_, middleware_impl) in middleware.iter_mut() {
                middleware_impl
                    .before_handle(boxed_event.event.as_mut())
            ...
            match self.call_handler_for_event(&*boxed_event.event, handler) {
                ...
                Err(e) => {
                    ...
                    let error_message = format!(
                        ...
                        handler_name, event_type_name, e
                    );
                    ...
                    details.insert(
                        ...
                        handler_processing_time.as_millis().to_string(),
                    );
        ...
        tokio::spawn(async move {
            ...
            for (handler_name, handler_time, handler_success) in handler_results {
                monitor
                    .record_event_completion(&handler_name, handler_time, handler_success)
                    .await;
            }
            ...
            if event_success {
                ...
                metadata.insert(
                    "processing_time_ms".to_string(),
                    total_processing_time.as_millis().to_string(),
                );
    ...
    pub async fn get_performance_summary(
        &self,
    ) -> Result<crate::events::performance_metrics::PerformanceSummary, EventError> {
        Ok(self.performance_monitor.get_performance_summary().await)
    }

    /// Check performance health status
    pub async fn check_performance_health(
        &self,
    ) -> Result<crate::events::performance_metrics::PerformanceHealthStatus, EventError> {
    ...
    pub async fn graceful_shutdown(
        &self,
    ) -> Result<(), crate::events::shutdown_manager::ShutdownError> {
        ...
        self.shutdown_manager
            .register_hook(event_bus_shutdown_hook)
            .await?;
        ...
        self.shutdown_manager
            .register_cleanup_task(queue_cleanup_task)
            .await?;
    ...
    fn apply_batch_middleware(
        &mut self,
        mut events: Vec<BoxedEvent>,
    ) -> Result<Vec<BoxedEvent>, EventError> {
        let mut middleware = self.middleware.lock().unwrap();

        for event in &mut events {
            for (_, middleware_impl) in middleware.iter_mut() {
                middleware_impl
                    .before_handle(event.event.as_mut())
                    .map_err(|e| EventError::MiddlewareError(e.to_string()))?;
            }
        }
    ...
    fn process_single_event_with_handlers(
        &mut self,
        event: BoxedEvent,
    ...
    pub fn optimize_memory(&mut self) -> Result<usize, EventError> {
        ...
        if queue_size > self.config.max_queue_size * 3 / 4 {
            ...
            tracing::info!(
                "Memory optimization cleaned {} old events from queue",
                cleaned_events
            );
        }
    ...
    pub fn handle_processing_error(
        ...
        if error_rate > 0.1 {
            ...
            tracing::error!(
                "High error rate detected ({}%), consider investigating event handlers",
                error_rate * 100.0
            );
