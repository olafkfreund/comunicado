...
impl EventMiddleware for PerformanceMiddleware {
    fn before_handle(&mut self, event: &mut dyn Event) -> Result<(), EventError> {
        ...
        self.active_events.lock().unwrap().insert(event_id, Instant::now());
        Ok(())
    }
    ...
    fn after_handle(&mut self, event: &dyn Event, _result: &Result<(), EventError>) {
        ...
        if let Some(start_time) = self.active_events.lock().unwrap().remove(&event_id) {
            ...
            // Update average
            metrics.average_processing_time = metrics.total_processing_time / metrics.total_events as u32;
            
            // Update per-event-type metrics
            *metrics.event_counts.entry(event_type.to_string()).or_insert(0) += 1;
            let event_total_time = metrics.event_times.entry(event_type.to_string()).or_insert(Duration::ZERO);
    ...
    fn before_handle(&mut self, event: &mut dyn Event) -> Result<(), EventError> {
        ...
        if let Some(validator) = self.validators.get(event_type) {
            if let Err(validation_error) = validator.validate(event) {
                let error_msg = format!("Validation failed for {}: {}", event_type, validation_error);
...
impl RateLimitMiddleware {
    ...
    pub fn add_source_limit(&mut self, source: String, max_events: u32, window: Duration) {
        self.rate_limits.insert(source, RateLimit::new(max_events, window));
    }
}
...
impl MiddlewareChain {
    ...
    pub fn with_logging(mut self, log_level: LogLevel) -> Self {
        self.middleware.push(Box::new(LoggingMiddleware::new(log_level)));
        self
    }
    
    pub fn with_performance_monitoring(mut self, slow_threshold: Duration) -> Self {
        self.middleware.push(Box::new(PerformanceMiddleware::new(slow_threshold)));
        self
    }
    
    pub fn with_debug_tracing(mut self) -> Self {
        self.middleware.push(Box::new(DebugMiddleware::new().enable_trace()));
        self
    }
    ...
    pub fn with_rate_limiting(mut self, max_events: u32, window: Duration) -> Self {
        self.middleware.push(Box::new(RateLimitMiddleware::new(max_events, window)));
        self
    }
