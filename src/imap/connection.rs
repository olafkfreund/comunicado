...
impl ImapConnection {
    ...
    pub async fn connect(&mut self) -> ImapResult<()> {
        ...
        let socket_addrs: Vec<_> = addr
            .to_socket_addrs()
            .map_err(|e| {
                ImapError::connection(format!("Failed to resolve address {}: {}", addr, e))
            })?
            .collect();
        ...
        tracing::info!(
            "Attempting TCP connection to {} (timeout: {}s)",
            addr,
    ...
    pub async fn send_command(&mut self, command: &str) -> ImapResult<String> {
        ...
        match self.stream.as_mut() {
            Some(SplitStream::Plain { writer, .. }) => {
                ...
                writer.flush().await.map_err(|e| {
                    ImapError::connection(format!("Failed to flush command: {}", e))
                })?;
            }
        ...
        loop {
            ...
            if let Some(literal_size) = Self::extract_literal_size(&line) {
                ...
                tracing::debug!(
                    "Read literal content, length: {} chars",
                    literal_string.len()
                );
            } else {
                responses.push(line.clone());
        ...
        match self.stream.as_mut() {
            Some(SplitStream::Plain { writer, .. }) => {
                writer.write_all(data.as_bytes()).await.map_err(|e| {
                    ImapError::connection(format!("Failed to send raw data: {}", e))
                })?;
                writer.flush().await.map_err(|e| {
                    ImapError::connection(format!("Failed to flush raw data: {}", e))
                })?;
            }
            Some(SplitStream::Tls { writer, .. }) => {
                writer.write_all(data.as_bytes()).await.map_err(|e| {
    ...
    pub async fn send_authenticate(
        ...
        tracing::debug!(
            "send_authenticate - Starting AUTHENTICATE {} with tag {}",
            mechanism,
            tag
        );
        ...
        tracing::debug!(
            "send_authenticate - Sending command: {}",
            auth_command.trim()
        );
        match self.stream.as_mut() {
            Some(SplitStream::Plain { writer, .. }) => {
                writer
                    ...
                    .await
                    .map_err(|e| {
                        ImapError::connection(format!("Failed to send authenticate command: {}", e))
                    })?;
                writer.flush().await.map_err(|e| {
                    ImapError::connection(format!("Failed to flush authenticate command: {}", e))
                })?;
            }
            Some(SplitStream::Tls { writer, .. }) => {
                writer
        ...
        if !continuation.starts_with("+ ") {
            return Err(ImapError::protocol(format!(
                "Expected continuation response, got: {}",
                continuation
            )));
        }
        ...
        tracing::debug!(
            "send_authenticate - Sending auth data (length: {})",
            auth_data.len()
        );
        ...
        loop {
            ...
            if line.starts_with(&tag) {
                ...
                if line.starts_with(&format!("{} OK", tag)) {
                    ...
                    return Err(ImapError::server(format!(
                        "Authentication failed: {}",
                        line
                    )));
                } else if line.starts_with(&format!("{} BAD", tag)) {
                    return Err(ImapError::protocol(format!(
                        "Bad authenticate command: {}",
                        line
                    )));
                }
                ...
                if !continuation_data.is_empty() {
                    ...
                    if let Ok(decoded_bytes) =
                        base64::prelude::BASE64_STANDARD.decode(continuation_data)
                    {
                        if let Ok(decoded_str) = String::from_utf8(decoded_bytes) {
                            tracing::debug!(
                                "send_authenticate - Decoded continuation: {}",
                                decoded_str
                            );
                            // Check if this looks like a JSON error response
                            if decoded_str.contains("\"status\"") && decoded_str.contains("400") {
                                tracing::debug!(
                                    "send_authenticate - Gmail returned error in continuation: {}",
                                    decoded_str
                                );
...
mod tests {
    ...
    fn test_predefined_configs() {
        ...
        let outlook_config =
            ImapConfig::outlook("user@outlook.com".to_string(), "password".to_string());
