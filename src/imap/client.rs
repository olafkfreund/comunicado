...
use crate::imap::{
    IdleNotification, IdleNotificationService, ImapAuthMethod, ImapCapability, ImapConfig,
    ImapConnection, ImapError, ImapFolder, ImapMessage, ImapResult, MessageFlag, SearchCriteria,
};
...
impl ImapClient {
    ...
    pub async fn authenticate(&mut self) -> ImapResult<()> {
        ...
        match &config.auth_method {
            ImapAuthMethod::OAuth2 { account_id } => {
                ...
                tracing::debug!(
                    "Using OAuth2 XOAUTH2 authentication for account: {}",
                    account_id
                );
                tracing::info!(
                    "Using OAuth2 XOAUTH2 authentication for account: {}",
                ...
                if !self.capabilities.contains(&ImapCapability::AuthXOAuth2) {
                    ...
                    tracing::error!(
                        "Server does not support XOAUTH2. Available capabilities: {:?}",
                        self.capabilities
                    );
                ...
                let response = self
                    .connection
                    .send_authenticate("XOAUTH2", &xoauth2_string)
                    .await
                    .map_err(|e| {
                ...
                tracing::info!(
                    "XOAUTH2 authentication successful. Response: {}",
            ...
            ImapAuthMethod::Password(password) => {
                ...
                if self.capabilities.contains(&ImapCapability::AuthPlain) {
                    let command =
                        ImapProtocol::format_authenticate_plain(&config.username, password)?;
    ...
    pub async fn list_folders(
        ...
        for folder in &folders {
            self.folders_cache
                .insert(folder.full_name.clone(), folder.clone());
        }
    ...
    pub async fn select_folder(&mut self, folder_name: &str) -> ImapResult<ImapFolder> {
        ...
        self.connection
            .set_state(ConnectionState::Selected(folder_name.to_string()));
        self.selected_folder = Some(folder_name.to_string());

        // Update cache
        self.folders_cache
            .insert(folder_name.to_string(), folder.clone());
    ...
    pub async fn uid_fetch_messages(
        ...
        tracing::debug!(
            "UID FETCH response first 500 chars: {}",
    ...
    pub async fn rename_folder(&mut self, old_name: &str, new_name: &str) -> ImapResult<()> {
        ...
        if let Some(folder) = self.folders_cache.remove(old_name) {
            ...
            self.folders_cache
                .insert(new_name.to_string(), updated_folder);
        }
    ...
    pub fn init_idle_service(&mut self) -> ImapResult<()> {
        ...
        let connection = Arc::new(Mutex::new(ImapConnection::new(
